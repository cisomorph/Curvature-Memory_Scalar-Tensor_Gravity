...
We subject RIFT to sixteen independent tests (Sims~87--103).
The key results are:
...
(xiv)~one-loop UV structure (Sim~102): quartic UV divergence confirmed; memory damping
    regulates to $\Sigma(0)=\Lambda_0^2 k_m^4/(64\pi^2)$; naturalness requires
    $\tau_m>0.11\,\mathrm{Mpc}$; UV finiteness holds at one loop in the
    non-minimal coupling sector;
(xv)~Vainshtein screening (Sim~103): all scanned $(\alpha_V,M_V,\beta)$
...
\begin{itemize}
  ...
  \item UV regulation (Sim~102): one-loop $\Psi$ self-energy from $\Lambda_0\Psi R$ vertex confirmed finite under memory damping --- $\Sigma(0) = \Lambda_0^2 k_m^4/(64\pi^2)$; quartic UV divergence regulated. Naturalness requires $\tau_m > 0.11\,\mathrm{Mpc}$. UV finiteness holds at one loop in the non-minimal coupling sector; higher loops and vertex renormalization remain open.
