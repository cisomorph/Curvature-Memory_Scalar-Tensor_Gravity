...
\date{July 2025 (v2, revised \today)}
...
Observational consistency tests against precision cosmological data (Sims~87--90) yield:
(i)~BAO full-covariance chi-squared $\chi^2/\mathrm{dof} = 9.78/8 = 1.22$ (BOSS DR12 + eBOSS DR16 + Ly-$\alpha$, 12 data points);
(ii)~RIFT CMB power spectrum computed via full CLASS Boltzmann solver deviates from $\Lambda$CDM by RMS~$= 2.93\%$ (l$=2$--1500), with gravitational growth ratio $D_{\mathrm{RIFT}}/D_{\Lambda\mathrm{CDM}} = 1.000000$ ($G_{\mathrm{eff}}/G = 1-16\,\mathrm{ppm}$ at $\Lambda_0=0.003$);
(iii)~joint CMB+BAO parameter fit gives $\Delta\chi^2(\mathrm{RIFT}-\Lambda\mathrm{CDM}) = 0.000$, confirming RIFT is observationally consistent with standard cosmology at this coupling. The framework is falsifiable through rotation curve reconstruction, void lensing phase shifts, and CMB low-$\ell$ suppression.
\end{abstract}
...
\begin{table}[ht]
\centering
\caption{Key observational results from RIFT simulations 87--90.}
\label{tab:results_summary}
\begin{tabular}{lllll}
\toprule
Observable & RIFT value & $\Lambda$CDM & Sim & Status \\
\midrule
BAO $\chi^2_{\mathrm{full}}/\mathrm{dof}$ & $9.78/8 = 1.22$ & $\sim$1.0 & 87 & Pass \\
...
$\Delta\chi^2$ joint CMB+BAO fit & $\mathbf{0.000}$ & baseline & 90 & \textbf{Pass} \\
...
% ============================================================
\section{Falsifiability and Observational Tests}
...
\subsection{CMB Low-$\ell$ Anomalies}
...
\begin{itemize}
  \item Unified Lagrangian (eq.~\ref{eq:action}) couples mass, curvature, and entropy through a single field $\Psi$.
  ...
  \item Observational consistency (Sims~87--90): BAO $\chi^2/\mathrm{dof} = 1.22$; CMB RMS deviation $= 2.93\%$; joint CMB+BAO $\Delta\chi^2 = 0$.
...
CMB+BAO dataset (Sim~90), model selection reduces to an Occam penalty calculation.
With $k$ free parameters and $N_{\mathrm{eff}} = 14$ data constraints (12 BAO points +
2 CMB Gaussian prior measurements), the information criteria give:
\begin{align}
...
is conventionally characterised as ``weak'' evidence (Jeffreys 1961), reflecting the
...
scales (structure formation, $\sigma_8$, galaxy cluster counts, void statistics)
at $\Lambda_0 \gtrsim 0.05$, and at higher coupling or in strong-field regimes
where $\Psi \gg \Psi_{\mathrm{ini}}$. Future simulations will:
\begin{enumerate}
  ...
  \item Compute $\sigma_8$ and non-linear $P(k)$ from RIFT structure formation at $\Lambda_0 = 0.05$;
