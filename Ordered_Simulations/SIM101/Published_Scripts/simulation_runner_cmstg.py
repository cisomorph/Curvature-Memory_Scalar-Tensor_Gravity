"""
SIM101 — CMSTG BAO Sound Horizon from First Principles
======================================================
Derives the BAO sound horizon r_d by integrating the CMSTG FLRW+scalar system
...
CMSTG modifications:
  (1) H(z) corrected by rho_Psi from the scalar stress-energy:
        ...
        H_CMSTG = H_LCDM * sqrt(1 + rho_Psi/rho_crit)
...
def R_b(z):
    """R_b = 3*rho_b / (4*rho_gamma); uses photon density only, not neutrinos."""
    return 0.75 * (Omega_b / Omega_gamma) / (1.0 + z)
...
def dPsi_dz(z, y, Lambda0=0.0):
    ...
    # Use CMSTG-corrected H in the ODE
    H_cmstg = H * np.sqrt(max(1.0 + f_Psi, 1.0))
    dH_cmstg_approx = dH * (1.0 + 0.5 * f_Psi)

    du_dz = (u * (2.0/(1.0+z) - dH_cmstg_approx / H_cmstg)
             - m0**2 * psi / ((1.0+z)**2 * H_cmstg**2))
    return [u, du_dz]
...
# ── CMSTG H_kms including rho_Psi ─────────────────────────────────────────────
def make_H_CMSTG_kms(sol_Psi):
    """Return a function H_CMSTG(z) [km/s/Mpc] using precomputed Psi solution."""
    def H_CMSTG_fn(z):
...
def make_cs2_CMSTG(sol_Psi, Lambda0):
...
for Lambda0 in Lambda0_scan:
    ...
    H_CMSTG_fn   = make_H_CMSTG_kms(sol)
    cs2_CMSTG_fn = make_cs2_CMSTG(sol, Lambda0)

    rd_val, rd_err = rd_integral(cs2_CMSTG_fn, H_CMSTG_fn, Z_DRAG)
