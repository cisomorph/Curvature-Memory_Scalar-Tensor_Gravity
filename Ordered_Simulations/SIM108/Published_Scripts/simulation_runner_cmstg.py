...
Derives the tensor perturbation equation from the linearised CMSTG action
...
Key analytic result:
  CMSTG belongs to Horndeski class with G5=0, which gives c_T=c exactly
...
print("="*70)
print("SIM108: CMSTG Gravitational Wave Speed and GW Constraints")
print("="*70)
print(f"Lambda0={Lambda0}, m0={m0} H0, Psi0={Psi0}\n")

print("─── 1. Tensor Perturbation Equation ───")
print("""
From linearising the CMSTG action around FLRW background:
...
Interpretation: c_T=c exactly because CMSTG has G5=0 in Horndeski notation.
The G5 term (phi-Gauss-Bonnet coupling) is the ONLY source of c_T != c
in Horndeski theory. Since CMSTG omits G5, the graviton speed is unchanged.
""")
...
The graviton mass m_g = 0 exactly in CMSTG.
...
def H_of_z(z):
    """Hubble parameter H(z)/H0 in flat LCDM (CMSTG background = LCDM at best-fit)."""
...
CMSTG modifies M_T relative to M_Pl. If Lambda0*Psi_inf^2 >> 1/(16piG):
  P_T^CMSTG / P_T^GR = (M_Pl / M_T(z_inf))^2
...
print()
print("Key insight: CMSTG is a Horndeski theory with G5=0.")
print("The graviton speed anomaly c_T != c requires non-zero G5.")
print("Since CMSTG has G5=0 by construction, c_T=c is exact and")
