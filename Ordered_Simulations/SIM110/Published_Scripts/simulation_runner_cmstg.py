#!/usr/bin/env python3
"""
SIM110: Slow-Roll Inflation in CMSTG
CMSTG: Curvature-Memory Scalar-Tensor Gravity

Analyses whether the locked CMSTG action (quadratic Jordan-frame potential)
supports a viable inflationary phase consistent with Planck CMB constraints.

CMSTG Einstein-frame potential:
...
print("="*70)
print("SIM110: Slow-Roll Inflation in CMSTG")
print("="*70)
print(f"Lambda0 = {Lambda0}")
print(f"Psi_max = 1/sqrt(2*Lambda0) = {Psi_max:.4f} M_Pl")
print()

# ─── Quadratic Jordan potential (locked CMSTG action) ─────────────────────
...
try:
    ...
    try:
        ...
        if not in_plateau:
            ...
            print(f"     i.e., Lambda0 >> {1/(2*P60_q**2):.2e} (CMSTG has Lambda0=0.003)")
            print(f"     Higgs inflation uses xi ~ 10^4-10^5; Starobinsky predictions")
            print(f"     are NOT attained with the CMSTG DE coupling Lambda0=0.003")
    except ValueError:
...
print()
models = [
    ("CMSTG quadratic, Lambda0=0.003",    0.9323, 0.01392,   "FAIL (n_s=0.932, hilltop eta)"),
    ("CMSTG quartic, Lambda0=0.003",      0.9312, 0.11972,   "FAIL (transition zone, not plateau)"),
    ("Quartic+large xi (Starobinsky)",   1-2/60, 12.0/3600, "PASS (xi>>1 needed; not CMSTG DE value)"),
...
print("SUMMARY")
print("="*70)
print(f"  Psi_max  = {Psi_max:.2f} M_Pl  (V_E maximum; hilltop of quadratic CMSTG potential)")
print(f"  N_max    = {N_max:.0f}  (numerically; analytic sub-Planck estimate: {1/(8*Lambda0):.0f})")
print()
print(f"  Locked CMSTG action (quadratic V_J = m^2*Psi^2/2):")
...
print()
print(f"  Quartic V_J = lambda*Psi^4 with Lambda0=0.003 (CMSTG DE value):")
...
print(f"    -> Not achieved with Lambda0=0.003 (the CMSTG DE coupling)")
print()
print(f"  Physical interpretation:")
print(f"    The CMSTG locked action (Lambda0=0.003) is an IR effective theory for DE.")
...
print()
print(f"  VERDICT:")
print(f"    Quadratic (locked CMSTG): FAIL (n_s=0.932, >5 sigma from Planck)")
...
diag = {
    ...
    'verdict_quartic_CMSTG'  : 'FAIL (n_s=0.931, transition zone; Lambda0=0.003 too small)',
    ...
    'conclusion'            : 'CMSTG DE coupling Lambda0=0.003 insufficient for inflation; UV sector needed',
}
