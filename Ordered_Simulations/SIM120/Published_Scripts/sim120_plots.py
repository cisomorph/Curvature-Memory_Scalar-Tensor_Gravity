"""
SIM120 / SIM120-alt — CMSTG Phase 2: Visualisation Suite
========================================================
Generates publication-quality figures for:
  1. H(z) comparison: CMSTG vs DESI BAO
  2. Ψ(a) field evolution (background quintessence)
  3. w(a) equation-of-state evolution
  4. κ–m₂₂ CMSTG DE-DM link diagram
...
# Standard flat ΛCDM (F0→0.5 i.e. no CMSTG correction) for reference
H_flat = H0_kms * np.sqrt([(Omega_m0*(1+z)**3 + Omega_r*(1+z)**4 + Omega_DE) for z in z_arr])

ax1.plot(z_arr, H_lcdm,    color='#2166ac', lw=1.8, ls='--', label=r'CMSTG frozen ($w=-1$)')
ax1.plot(z_arr, H_sim113,  color='#d6604d', lw=2.0, label=r'CMSTG SIM113 ($w_0=-0.973,\,w_a=-0.41$)')
ax1.plot(z_arr, H_flat,    color='gray',    lw=1.2, ls=':', alpha=0.7, label=r'Flat $\Lambda$CDM (no CMSTG $F$)')
...
ax1.set_title('SIM120 — CMSTG Phase 2: $H(z)$ vs DESI BAO', fontsize=13)
ax1.set_xticklabels([])
...
if a_bg is not None:
    ...
    ax1.text(0.03, 0.92, 'CMSTG Phase 2\n' + r'$V_J = \lambda(\Psi^2-v^2)^2$',
             transform=ax1.transAxes, fontsize=9, va='top',
             bbox=dict(boxstyle='round', fc='white', alpha=0.8))

    # w(a) — only show late-time (a > 0.1) where quintessence matters
    mask = a_bg > 0.05
    ax2.plot(a_bg[mask], w_bg[mask], color='#7b2d8b', lw=2, label='CMSTG numerical')
    # CPL overlay
    a_cpl = a_bg[mask]
...
ax.loglog(m22_range, kappa_arr, color='#2166ac', lw=2.5,
          label=r'$\kappa = \frac{m_\chi^2}{2\bar\Psi^2 M_{\rm Pl}^2}$  (CMSTG link)')
...
ax.set_title(r'SIM120 — CMSTG DE-DM Link: $m_\chi = \sqrt{2\kappa}\,\bar\Psi$', fontsize=13)
...
table_data = [
    ...
    ['SIM118',      r'CMSTG-seeded fuzzy $\chi$ DM',  'PARTIAL', r'$m_{22}=0.082$, 18 PASS, $\chi^2/{\rm dof}=0.34$'],
...
ax.set_title('CMSTG Phase 2 — Simulation Summary (SIM112–SIM120)', fontsize=13, pad=8)
fig.tight_layout()
