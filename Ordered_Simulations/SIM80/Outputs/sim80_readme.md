...
- CMSTG propagator: $G(p) = 1/(p^2 + m^2 + \Sigma_{\rm mem}(p))$
...
- Bubble integral growth vs cutoff:
  ...
  - CMSTG shows visibly softened growth across the same Λ range due to the positive memory term at internal momenta below $\Lambda_{\rm mem}$.
...
- `Published_Scripts/simulation_runner_cmstg.py` — Re-run script
