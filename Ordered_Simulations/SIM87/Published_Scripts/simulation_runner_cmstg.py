#!/usr/bin/env python3
# Sim 87 — CMSTG BAO Full Covariance Refit
"""
...
CMSTG background model (FLRW, from section3_lagrangian.tex, verified SIM85-86):
...
#
# CMSTG field: Psi dimensionless, Lambda0 dimensionless,
# m0 in units of H0 (so m_phys = m0 * H0)

def integrate_cmstg_background(H0, Omega_m, Lambda0, Psi_ini=0.01,
                               m0=1.0, alpha=0.1, beta=0.05,
                               Omega_b=0.049, Omega_r=9.2e-5,
                               a_ini=1e-5, a_fin=1.0, N=5000):
    """
    Integrate the CMSTG FLRW system in ln(a) coordinates.
    ...
    Omega_L = 1.0 - Omega_m - Omega_r   # flat universe, CMSTG field is extra
...
def predict_bao(H0, Omega_m, r_d, Lambda0,
                ...
                Omega_b=0.049, Omega_r=9.2e-5):
    """
    Run the CMSTG background and return model predictions for the BAO data vector.
    Returns a 12-element numpy array aligned with BAO_DATA ordering.
    Returns None if integration failed.
    """
    try:
        a_arr, H_arr, _ = integrate_cmstg_background(
...
diag_out = {
    ...
    "comparison": {
        ...
        "interpretation": (
            ...
            "when DM-DH residuals have opposite sign (expected for typical CMSTG vs LCDM). "
