#!/usr/bin/env python3
# Sim 90 — CMSTG Joint CMB+BAO Parameter Fit
"""
Resolves the Phase 5b finding (SIM89): the CMSTG BAO-only best-fit
...
gives CMSTG a small additional BAO degree of freedom; the question is whether
Δchi2(CMSTG-LCDM) < 9 (3-sigma, 1 extra dof).
...
# ── CMSTG H(z) — flat Friedmann with G_eff correction ────────────────────────
...
def H_flat(H0_kms, Omega_m, z, Lambda0=0.0):
    """H(z) [km/s/Mpc]. Flat Friedmann with CMSTG G_eff correction at Psi=Psi0."""
    a = 1.0 / (1.0 + z)
...
def DC_flat(H0_kms, Omega_m, z_target, Lambda0=0.0, N=600):
    """Comoving distance D_C(z) [Mpc] for flat cosmology with CMSTG G_eff."""
...
def predict_bao(H0_kms, Omega_m, Lambda0, rd):
    """
    Predict 12-element BAO data vector with CMSTG G_eff correction.
...
print("  Optimising CMSTG joint fit (Lambda0 free) ...", flush=True)
chi2_cmstg_best, x_cmstg = _run_optim(fix_lambda0=None)
H0_cmstg  = float(x_cmstg[0])
Omm_cmstg = float(x_cmstg[1])
rd_cmstg  = float(x_cmstg[2])
L0_cmstg  = float(x_cmstg[3])

delta_chi2 = chi2_cmstg_best - chi2_lcdm_best
tension_sigma = math.sqrt(abs(delta_chi2)) if delta_chi2 > 0 else 0.0
passed = bool(delta_chi2 < float(P["acceptance"]["Delta_chi2_CMSTG_minus_LCDM_max"]))

# Decompose chi2
chi2_cmb_lcdm = chi2_cmb(H0_lcdm, Omm_lcdm)
chi2_bao_lcdm = chi2_bao(H0_lcdm, Omm_lcdm, 0.0,    rd_lcdm)
chi2_cmb_cmstg = chi2_cmb(H0_cmstg,  Omm_cmstg)
chi2_bao_cmstg = chi2_bao(H0_cmstg,  Omm_cmstg, L0_cmstg, rd_cmstg)
...
print(f"  CMSTG: H0={H0_cmstg:.3f}, Omm={Omm_cmstg:.4f}, rd={rd_cmstg:.2f}, L0={L0_cmstg:.5f}  chi2={chi2_cmstg_best:.3f}")
print(f"        chi2_CMB={chi2_cmb_cmstg:.3f}  chi2_BAO={chi2_bao_cmstg:.3f}")
print(f"  Δchi2 (CMSTG−LCDM) = {delta_chi2:+.4f}  ({tension_sigma:.3f}σ)  {'PASS' if passed else 'FAIL'}")
...
for ax, (Z, label, best_H0, best_Omm) in zip(axes, [
    ...
    (Z_cmstg, "CMSTG (Λ₀=0.003)", H0_cmstg, Omm_cmstg),
]):
    Zmin = Z.min()
...
plt.suptitle(f"Joint CMB+BAO chi2 landscape  |  Δchi2(CMSTG−LCDM)={delta_chi2:+.4f}  ({tension_sigma:.3f}σ)",
             fontsize=10, y=1.01)
plt.tight_layout()
p_landscape = os.path.join(OUTPUTS, "sim90_chi2_landscape.png")
fig.savefig(p_landscape, bbox_inches="tight"); plt.close(fig)

# Plot 2: chi2 vs Lambda0 scan at joint CMSTG best-fit (H0, Omm)
L0_scan = np.linspace(0, 0.025, 80)
chi2_bao_vs_L0 = [chi2_bao(H0_cmstg, Omm_cmstg, L0, rd_cmstg) for L0 in L0_scan]
chi2_tot_vs_L0 = [chi2_cmb_cmstg + c for c in chi2_bao_vs_L0]
...
ax.axvline(L0_cmstg, color="gray", ls=":", lw=1.2,
           label=f"CMSTG best Λ₀ = {L0_cmstg:.4f}")
...
ax.set_title(f"Sim 90 — BAO chi2 vs Λ₀  at joint best-fit H₀={H0_cmstg:.2f}, Ωm={Omm_cmstg:.4f}, rd={rd_cmstg:.2f}")
...
vR   = [chi2_cmb_cmstg,  chi2_bao_cmstg,  chi2_cmstg_best]
x    = np.arange(len(cats))
w    = 0.35
ax.bar(x - w/2, vL, w, label="LCDM",                     color="steelblue", alpha=0.85)
ax.bar(x + w/2, vR, w, label=f"CMSTG (Λ₀={L0_cmstg:.4f})", color="tomato",    alpha=0.85)
...
diag = {
    "description": (
        "CMSTG joint CMB+BAO parameter fit (SIM90). Resolves Phase 5b (SIM89: 30.1-sigma tension). "
    ...
    "bao_model_note": (
        ...
        "where Omega_Lambda = 1 - Omega_m - Omega_r. The CMSTG G_eff correction (16 ppm "
    ...
    "CMSTG_joint": {
        "H0":        H0_cmstg,  "Omega_m": Omm_cmstg,
        "rd":        rd_cmstg,  "Lambda0": L0_cmstg,
        "chi2_CMB":  float(chi2_cmb_cmstg),
        "chi2_BAO":  float(chi2_bao_cmstg),
        "chi2_total": float(chi2_cmstg_best),
    },
    "comparison": {
        ...
        "passed":         passed,
        "interpretation": (
            f"Delta_chi2 = chi2_CMSTG_joint - chi2_LCDM_joint = {delta_chi2:+.4f}. "
            ...
            "the Planck LCDM best-fit for both CMSTG and LCDM. Lambda0 contribution to "
            "chi2_BAO is small because G_eff/G = 1-16ppm at Lambda0=0.003, so CMSTG H(z) "
    ...
    "sim89_tension_context": (
        f"SIM89 found Δchi2=+906.85 (30.1-sigma) for CMSTG BAO-only best-fit vs Planck CMB. "
        f"That used the wrong reference (Dl_CMSTG vs Planck spectrum, not parameter comparison). "
        ...
        "relevant comparison (model selection between CMSTG and LCDM given both datasets)."
    ),
    "artifacts": {
...
print(f"  Joint Δchi2(CMSTG−LCDM) = {delta_chi2:+.4f}  ({tension_sigma:.3f}σ)  {'PASS' if passed else 'FAIL'}")
print(f"  CMSTG joint best-fit: H0={H0_cmstg:.3f}, Omega_m={Omm_cmstg:.4f}, rd={rd_cmstg:.2f}, Lambda0={L0_cmstg:.5f}")
