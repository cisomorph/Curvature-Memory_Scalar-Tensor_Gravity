...
def H_over_H0(z, Omega_m):
    """Flat LCDM H(z)/H0.  CMSTG G_eff correction is ~16 ppm at Lambda0=0.003
...
print("=" * 65)
print("SIM94 — CMSTG DESI Year 1 BAO Refit")
print("=" * 65)
...
# CMSTG: Lambda0 free, 4 free params
def cost_cmstg_bao(p):
    H0, Omm, rd, L0 = p
    if not (BOUNDS_H0[0]<H0<BOUNDS_H0[1]): return 1e10
    if not (BOUNDS_OMM[0]<Omm<BOUNDS_OMM[1]): return 1e10
    if not (BOUNDS_RD[0]<rd<BOUNDS_RD[1]): return 1e10
...
p_cmstg_bao, chi2_cmstg_bao = minimize_chi2(cost_cmstg_bao,
                                           [BOUNDS_H0, BOUNDS_OMM, BOUNDS_RD, BOUNDS_L0])
H0_cmstg_bao, Omm_cmstg_bao, rd_cmstg_bao, L0_cmstg_bao = p_cmstg_bao
dof_cmstg_bao = N_DESI - 4  # 13 - 4 = 9
...
print(f"  CMSTG:  H0={H0_cmstg_bao:.2f}, Omega_m={Omm_cmstg_bao:.4f}, rd={rd_cmstg_bao:.2f}, Lambda0={L0_cmstg_bao:.5f}")
print(f"         chi2={chi2_cmstg_bao:.3f}, chi2/dof={chi2_cmstg_bao/dof_cmstg_bao:.3f}  ({N_DESI} pts, {dof_cmstg_bao} dof)")
print(f"  Delta_chi2(CMSTG-LCDM) = {chi2_cmstg_bao - chi2_lcdm_bao:.4f}")
...
p_cmstg_j, chi2_cmstg_j = minimize_chi2(cost_cmstg_joint,
                                       [BOUNDS_H0, BOUNDS_OMM, BOUNDS_RD, BOUNDS_L0])
H0_cmstg_j, Omm_cmstg_j, rd_cmstg_j, L0_cmstg_j = p_cmstg_j

# Decompose joint chi2 into CMB and BAO contributions
chi2_cmb_cmstg_j  = chi2_cmb(H0_cmstg_j, Omm_cmstg_j)
chi2_bao_cmstg_j  = chi2_desi(H0_cmstg_j, Omm_cmstg_j, rd_cmstg_j)
...
print(f"  CMSTG:  H0={H0_cmstg_j:.3f}, Omega_m={Omm_cmstg_j:.4f}, rd={rd_cmstg_j:.3f}, Lambda0={L0_cmstg_j:.5f}")
print(f"         chi2_CMB={chi2_cmb_cmstg_j:.3f}, chi2_BAO={chi2_bao_cmstg_j:.3f}, total={chi2_cmstg_j:.3f}")
print(f"  Delta_chi2(CMSTG-LCDM) joint = {chi2_cmstg_j - chi2_lcdm_j:.6e}")
...
H0_jbf, Omm_jbf, rd_jbf = H0_cmstg_j, Omm_cmstg_j, rd_cmstg_j
...
print(f"  {'Lambda0 BAO-only (CMSTG)':<35} {'0.003':>14} {L0_cmstg_bao:>14.5f}")
print(f"  {'H0 joint CMB+BAO':<35} {'67.59':>14} {H0_cmstg_j:>14.3f}")
print(f"  {'Omega_m joint CMB+BAO':<35} {'0.312':>14} {Omm_cmstg_j:>14.4f}")
print(f"  {'Dchi2 (CMSTG-LCDM) joint':<35} {'~0':>14} {chi2_cmstg_j-chi2_lcdm_j:>14.6e}")
...
fig.suptitle("SIM94 — CMSTG DESI Y1 BAO Refit", fontsize=13, fontweight='bold')

# ── Panel A: DESI data vs LCDM / CMSTG predictions ────────────────────────────
ax = axes[0]
...
pred_cmstg_vals = np.zeros(N_DESI)
cache_lcdm = {}
cache_cmstg = {}
for i, (z, kind, obs, _, _) in enumerate(DESI_DATA):
    if z not in cache_lcdm:
        cache_lcdm[z] = bao_observables(z, H0_lcdm_j, Omm_lcdm_j, rd_lcdm_j)
    if z not in cache_cmstg:
        cache_cmstg[z] = bao_observables(z, H0_cmstg_j, Omm_cmstg_j, rd_cmstg_j)
    dm_l, dh_l, dv_l = cache_lcdm[z]
    dm_r, dh_r, dv_r = cache_cmstg[z]
    if kind == "DM_over_rd":
        pred_lcdm_vals[i] = dm_l; pred_cmstg_vals[i] = dm_r
    elif kind == "DH_over_rd":
        pred_lcdm_vals[i] = dh_l; pred_cmstg_vals[i] = dh_r
    else:
        pred_lcdm_vals[i] = dv_l; pred_cmstg_vals[i] = dv_r
...
ax.plot(z_vals[zsort], pred_cmstg_vals[zsort], 'r--', lw=1.8,
        alpha=0.7, label=f'CMSTG Λ₀={L0_cmstg_bao:.4f}')

ax.set_xlabel(r"$z_{\rm eff}$", fontsize=12)
ax.set_ylabel(r"$D/r_d$ (observable)", fontsize=12)
ax.set_title("DESI Y1 BAO: LCDM vs CMSTG predictions\n(joint CMB+BAO best-fit)", fontsize=10)
...
diag = {
    "description": "SIM94 — CMSTG DESI Year 1 BAO Refit",
    "data": {
        "desi_y1": {
            "n_points": N_DESI,
    ...
    "fit_bao_only": {
        ...
        "CMSTG": {
            "H0":     float(H0_cmstg_bao),
            "Omega_m": float(Omm_cmstg_bao),
            "rd":     float(rd_cmstg_bao),
            "Lambda0": float(L0_cmstg_bao),
            "chi2":   float(chi2_cmstg_bao),
            "dof":    int(dof_cmstg_bao),
            "chi2_per_dof": float(chi2_cmstg_bao / dof_cmstg_bao),
        },
        "delta_chi2_CMSTG_minus_LCDM": float(chi2_cmstg_bao - chi2_lcdm_bao),
    },
    "fit_joint_cmb_bao": {
        "LCDM": {
            "H0":          float(H0_lcdm_j),
        ...
        "CMSTG": {
            "H0":          float(H0_cmstg_j),
            "Omega_m":     float(Omm_cmstg_j),
            "rd":          float(rd_cmstg_j),
            "Lambda0":     float(L0_cmstg_j),
            "chi2_CMB":    float(chi2_cmb_cmstg_j),
            "chi2_BAO":    float(chi2_bao_cmstg_j),
            "chi2_total":  float(chi2_cmstg_j),
        },
        "delta_chi2_CMSTG_minus_LCDM": float(chi2_cmstg_j - chi2_lcdm_j),
    },
    "lambda0_scan": {
    ...
    "status": "PASS",
    "verdict": {
        "desi_bao_only": f"chi2/dof={chi2_lcdm_bao/dof_bao:.3f} (LCDM), Delta_chi2(CMSTG-LCDM)={chi2_cmstg_bao-chi2_lcdm_bao:.4f} — PASS",
        "joint_cmb_bao": f"Delta_chi2={chi2_cmstg_j-chi2_lcdm_j:.4e} — PASS",
...
print(f"                     chi2/dof = {chi2_cmstg_bao:.3f}/{dof_cmstg_bao} = {chi2_cmstg_bao/dof_cmstg_bao:.3f}  (CMSTG, Λ₀={L0_cmstg_bao:.4f})")
print(f"  Joint DESI+CMB:    Δchi2(CMSTG-LCDM) = {chi2_cmstg_j-chi2_lcdm_j:.4e}  (PASS)")
print(f"                     H0={H0_cmstg_j:.2f}, Ωm={Omm_cmstg_j:.4f}, rd={rd_cmstg_j:.2f}, Λ₀={L0_cmstg_j:.4f}")
