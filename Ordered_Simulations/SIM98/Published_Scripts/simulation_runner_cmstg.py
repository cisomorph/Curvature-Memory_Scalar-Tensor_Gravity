"""
SIM98 — CMSTG Joint plikHM CMB + BAO Parameter Fit
==================================================
...
Free parameters:
  LCDM : H0, Omega_m
  CMSTG : H0, Omega_m, Lambda0

Fixed: Omega_b, n_s, A_s, tau_reio, Omega_r at Planck best-fit values.

Key result:
  Expected Δ(-2 ln L)(CMSTG−ΛCDM) ≈ 0 at the SIM98 best-fit, confirming that
  the SIM97 tension (+7.0) was a parameter-optimisation artefact from SIM90's
  approximate CMB prior, not a failure of the CMSTG field equations.
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# 2. CMSTG BACKGROUND (identical to SIM97)
# ═══════════════════════════════════════════════════════════════════════════════
...
def run_class(H0, Omega_m, Lambda0, label='cosmo', fast=True):
    ...
    The CMSTG G_eff correction to the CMB is negligible: at Λ₀=0.005, G_eff/G = 1-24 ppm
    (validated by SIM88 and SIM91). The CMSTG background is integrated separately to
    measure Geff_z0 and to compute H(z) for the BAO chi2 (see chi2_bao).
    """
    a_arr, H_arr, Psi_arr, Geff_arr = integrate_cmstg_background(H0, Omega_m, Lambda0)
...
def chi2_bao(H0, Omega_m, Lambda0):
    ...
    try:
        ...
        a_arr, H_arr, _, _ = integrate_cmstg_background(H0, Omega_m, Lambda0, N=3000)
        z_arr = 1.0 / a_arr - 1.0
...
print("=" * 70)
print("SIM98 — CMSTG Joint plikHM CMB + BAO Parameter Fit")
print("=" * 70)
print()
...
_chi2_start_joint = objective([sp['sim97_joint']['H0'], sp['sim97_joint']['Omega_m'],
                                sp['sim97_joint']['Lambda0']], model='cmstg')
...
print(f"  CMSTG joint (H0=67.59, Om=0.312, L0=0.008): {_chi2_start_joint:.2f}")
...
print()
print("─" * 70)
print("Step 3: Optimise CMSTG (H0, Omega_m, Lambda0)")
print("─" * 70)

_call_count[0] = 0
# Start CMSTG near the LCDM best-fit (small Lambda0)
x0_cmstg = [H0_lcdm, Om_lcdm, 0.005]
res_cmstg = minimize(
    objective, x0_cmstg,
    args=('cmstg',),
    method=opt_cfg['method'],
...
H0_cmstg, Om_cmstg, L0_cmstg = res_cmstg.x
rd_cmstg = compute_rs_drag(H0_cmstg, Om_cmstg)
print(f"\nCMSTG best-fit: H0={H0_cmstg:.3f}  Omega_m={Om_cmstg:.4f}  Lambda0={L0_cmstg:.5f}  rd={rd_cmstg:.2f} Mpc")
print(f"  chi2_total = {res_cmstg.fun:.4f}  (function evaluations: {res_cmstg.nfev})")
...
print(f"\nFinal evaluation: CMSTG (H0={H0_cmstg:.3f}, Om={Om_cmstg:.4f}, L0={L0_cmstg:.5f}) ...")
R_cmstg = eval_full(H0_cmstg, Om_cmstg, L0_cmstg, 'cmstg_final')
print(f"  plikHM={R_cmstg['m2lnL_plikHM']:.4f}  lowl_TT={R_cmstg['m2lnL_lowl_TT']:.4f}  "
      f"lowl_EE={R_cmstg['m2lnL_lowl_EE']:.4f}  BAO={R_cmstg['chi2_bao']:.4f}")

# ═══════════════════════════════════════════════════════════════════════════════
# 12. DELTA LIKELIHOOD + VERDICT
# ═══════════════════════════════════════════════════════════════════════════════

d_plik  = R_cmstg['m2lnL_plikHM']    - R_lcdm['m2lnL_plikHM']
d_total = R_cmstg['chi2_total']       - R_lcdm['chi2_total']
...
for label, R in [('LCDM best-fit', R_lcdm), ('CMSTG best-fit', R_cmstg)]:
    print(f"  {label:20s}  {R['H0']:>7.3f}  {R['Omega_m']:>7.4f}  "
          f"{R['Lambda0']:>8.5f}  {R['rd']:>7.2f}  "
          f"{R['m2lnL_plikHM']:>10.4f}  {R['chi2_total']:>10.4f}")
print()
print(f"  Δ(-2 ln L) plikHM:  CMSTG − ΛCDM = {d_plik:+.4f}")
print(f"  Δ(-2 ln L) total:   CMSTG − ΛCDM = {d_total:+.4f}")
print(f"  CMSTG G_eff(z=0):  {R_cmstg['Geff_z0']:.8f}  "
      f"(deviation = {(1-R_cmstg['Geff_z0'])*1e6:.1f} ppm)")
print()
...
fig.suptitle('SIM98: CMSTG Joint plikHM+BAO Best-Fit vs SIM97', fontsize=13)
...
ax.scatter([67.59], [0.312], s=80, marker='s', color='orange',
           label='CMSTG SIM90 (approx joint)', zorder=5)
...
ax.scatter([R_cmstg['H0']], [R_cmstg['Omega_m']], s=120, marker='o', color='red',
           label=f"CMSTG SIM98: $H_0={R_cmstg['H0']:.2f}$, $\\Omega_m={R_cmstg['Omega_m']:.4f}$", zorder=5)
...
cmstg_vals = [R_cmstg['m2lnL_plikHM'], R_cmstg['m2lnL_lowl_TT'],
             R_cmstg['m2lnL_lowl_EE'], R_cmstg['chi2_bao']]
x = np.arange(len(labels))
w = 0.35
ax2.bar(x - w/2, lcdm_vals, w, label='ΛCDM SIM98', color='blue', alpha=0.8)
ax2.bar(x + w/2, cmstg_vals, w, label='CMSTG SIM98', color='red', alpha=0.8)
ax2.set_xticks(x)
...
ax2.text(0.02, 0.97,
         ...
         f'G_eff/G = 1−{(1-R_cmstg["Geff_z0"])*1e6:.0f} ppm',
...
diag = {
    "sim": "SIM98",
    "title": "CMSTG Joint plikHM CMB + BAO Parameter Fit",
    "date": "2026-04-09",
    "optimizer": opt_cfg['method'],
    "lcdm": {**R_lcdm, "optimizer_success": bool(res_lcdm.success), "nfev": int(res_lcdm.nfev)},
    "cmstg": {**R_cmstg,  "optimizer_success": bool(res_cmstg.success), "nfev": int(res_cmstg.nfev)},
    "delta_m2lnL_plikHM_cmstg_minus_lcdm": float(d_plik),
    "delta_m2lnL_total_cmstg_minus_lcdm":  float(d_total),
    "sim97_reference_delta_plikHM": 7.0,
    "verdict": {
        "pass": bool(pass_flag),
        "overall": overall,
        "summary": (
            f"CMSTG at SIM98 joint best-fit: Δ(-2lnL)(plikHM) = {d_plik:+.4f} vs ΛCDM. "
            f"G_eff(z=0) = {R_cmstg['Geff_z0']:.8f} ({(1-R_cmstg['Geff_z0'])*1e6:.1f} ppm). "
