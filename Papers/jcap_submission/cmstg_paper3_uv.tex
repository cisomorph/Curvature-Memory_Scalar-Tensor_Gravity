...
feature of the scalar sector and characterised by a physical memory scale $\km$
whose normalisation carries an $O(1)$ ambiguity from the choice of kernel shape
within the universality class (SIM102b, \S\ref{sec:kernel}).
...
$\knat = 9.15\,\mathrm{Mpc}^{-1}$\footnote{Computed with the simulation scalar
mass $m_0 = 0.01\,\mathrm{Mpc}^{-1}$ (SIM90/SIM102). The cosmological locked
value $m_0 = 10^{-3}H_0 \approx 2.3\times10^{-7}\,\mathrm{Mpc}^{-1}$ gives a
much smaller naturalness threshold; $m_0 = 0.01\,\mathrm{Mpc}^{-1}$ is the
relevant mass parameter throughout the UV loop analysis.}.
...
infrared Landau pole; we refer to this as the \emph{$\Lz$ fixed point}.
...
Companion papers establish (I)~the no-go theorems and their exhaustive
mechanism-completeness proofs (SIM131--SIM144)~\cite{WilsonPaperI},
(II)~the framework and observational consistency~\cite{WilsonPaperII}, and
(IV)~galactic-scale constraints~\cite{WilsonPaperIV}.
\end{abstract}

\keywords{scalar-tensor gravity; UV finiteness; $\Lz$ fixed point; renormalization group flow; memory kernel; loop corrections}
...
constraints on its dark-energy phenomenology, including an exhaustive
mechanism-completeness search (SIM131--SIM144) confirming the DESI
residual is irreducible within the \CMSTG\ action class.
...
We commit to the \emph{effective theory in the cosmological rest frame}
interpretation: the loop calculations of this paper are performed in the CMB
rest frame, with $\km$ a physical scale defined in that frame, and no further
covariance argument is required for any result reported here. This is not
unusual in cosmological EFT. The EFT of Inflation~\cite{Cheung2008} and the
EFT of Dark Energy~\cite{Gubitosi2013} both proceed by breaking full 4d
diffeomorphism invariance to spatial diffeomorphisms in the cosmological rest
frame; operators are organised in the unitary gauge that selects this frame,
the preferred frame is physically motivated by the CMB, and the loop structure
is correspondingly frame-dependent. \CMSTG\ adopts the same stance: the kernel
$e^{-k^2/\km^2}$ encodes the finite resolving power of $\Psi$ in the cosmic
fluid rest frame, all loop integrals are integrals in that frame, and the
theory is understood as an effective description valid on a fixed cosmological
background.

A fully covariant completion---promoting $\km$ to a scalar constructed from
background invariants such as $\Box\Psi$ or $R$ and transforming as a scalar
under diffeomorphisms---is left for future work. It would reduce to the present
theory on the FLRW background and would not alter any result in this paper.
...
\textbf{(d) Frame dependence and scope of the kernel postulate.}
The Gaussian factor $e^{-k^2/\km^2}$ in~\eqref{eq:propagator} is written in
the cosmological rest frame---the frame in which the CMB dipole vanishes.
This is a defining feature of the effective theory, not a Lorentz-invariant
statement: $k^2$ denotes the Euclidean momentum magnitude evaluated in that
frame, and the kernel suppresses modes that are energetically hard relative to
the cosmic fluid. The kernel does not transform as a Lorentz scalar, and the
theory is not intended to be Lorentz-invariant at the level of the propagator.
This is explicit in subsection~(b) above and is restated here for emphasis
because it is the most natural target for a referee objection.

This class of frame-dependent choice is not novel in non-local gravitational
effective theories, and precedent from the literature shows that it is
compatible with a predictive and testable EFT.
Barvinsky and Vilkovisky~\cite{Barvinsky1985} formulate non-local form factors
for the heat-kernel expansion of one-loop gravity relative to a fixed background
field configuration; the non-local operators are evaluated in a frame selected
by that background, breaking full Lorentz covariance at the level of the
propagator while preserving diffeomorphism covariance of the effective action.
Donoghue~\cite{Donoghue1994} treats GR as a Wilsonian effective field theory
with a UV cutoff that is implicitly frame-dependent once matter sources are
included; the leading quantum corrections he computes are nonetheless
unambiguous physical predictions.
Maggiore and Mancarella~\cite{MaggioreMC2014} introduce a non-local dark-energy
model via the term $m^2 R\,\Box^{-2}R$ evaluated on a fixed FLRW background;
the predictivity of the model rests on evaluating the non-local operator in
the frame in which that background is isotropic, which is the cosmological rest
frame.
Belgacem et al.~\cite{Belgacem2018} extend this programme and demonstrate that
frame-dependent non-local EFTs of this type produce observationally distinguishable
predictions consistent with current data, establishing the general programme
as viable.
\CMSTG\ adopts the same stance: the cosmological rest frame is physically
selected by the CMB, the kernel encodes the finite resolving power of $\Psi$
in that frame, and frame-dependence at the level of the propagator is an
explicit feature rather than a defect of the effective description.

The observable predictions of Papers~I, II, and~IV are insensitive to this
frame choice at the precision of current data. All observational tests in
those papers---the no-go theorems and their simulation probes (SIM131--SIM144),
the cosmological consistency checks, and the galactic-scale rotation-curve
analysis---are performed in the CMB rest frame. The frame in which the kernel
is defined is precisely the frame in which all current data are gathered.
Frame ambiguity therefore does not propagate to any prediction at current
observational precision: there is no physically accessible alternative frame
in which the tests of Papers~I--II--IV are conducted that would require a
different kernel evaluation.

A fully covariant promotion of $\km$---replacing it with a scalar constructed
from background invariants such as $\km(R)$ or $\km(\Box\Psi)$ that transforms
as a scalar under full diffeomorphisms---is left for future work and may require
heat-kernel methods on the curved background following
Barvinsky and Vilkovisky~\cite{Barvinsky1985}. On an FLRW background any such
promotion reduces to the present theory and would not alter any result in this
paper. We flag this as a Phase~5 direction (see also Paper~IV's discussion of
dark-matter extensions~\cite{WilsonPaperIV}).
...
exactly (not a normalisation artefact: $\Gamma(1) = 1$ and $2^{2/2-1} = 1$
independently force $c_2 = 1$; the same mechanism gives $c_1 = 1$ via $\Gamma(2) = 1$),
$c_3 \approx 1.137$, and $c_4 \approx 1.253$. The
...
This universality has a direct implication for the status of $\km$ as a
physical scale: its normalisation is not uniquely fixed by the physics, but
depends at $O(1)$ on the kernel shape within the universality class.
Concretely, $\km$ and $c_n^{1/4}\km$ are physically equivalent descriptions
of the same memory scale for different choices of $n$; the naturalness bound
$\knat = 9.15\,\mathrm{Mpc}^{-1}$, computed for the Gaussian kernel ($n=1$),
carries a corresponding $O(1)$ uncertainty from kernel shape. The claim that
the theory is characterised by a physical memory scale $\km$ should be
understood in this sense: $\km$ is the single relevant UV scale of the theory,
but its precise normalisation is kernel-shape-ambiguous at $O(1)$.

This ambiguity does not, however, undermine the renormalization-group result
of Section~\ref{sec:rg}. The beta function $\beta(\Lz) = -\Lz^3\km^2/(16\pi^2
m_0^2)$ scales uniformly as $\km^2$; an $O(1)$ rescaling of $\km$ rescales the
RG speed by the same $O(1)$ factor but does not change the \emph{sign} of
$\beta$, the existence of the UV fixed point at $\Lz = 0$, or the absence of
an IR Landau pole. The qualitative structure of the RG trajectory---$\Lz$
small, positive, and decreasing toward the UV with no Landau pole---is
kernel-shape-independent. What carries the $O(1)$ ambiguity is the precise
numerical value of the fractional running $(\Lz(0) -
\Lz_{\rm obs})/\Lz_{\rm obs} = 2.47\%$, which would shift by an $O(1)$ factor
under a different kernel normalisation but would remain small in either case.
...
\emph{fixed} memory scale $\km$. Unlike standard renormalization-group running,
$\km$ is a fixed input of the theory and does not itself run; the beta function
describes the probe-scale sensitivity of $\Lz$ as $\mu$ approaches $\km$ from
below. The \emph{$\Lz$ fixed point} $\Lz \to 0$ is the limit in which $\Lz$
is probed at the memory scale itself, where the kernel becomes maximally
effective. The physical content: $\Lz$ is small, positive, and decreasing
toward the UV, with no Landau pole at any accessible scale.
...
General Relativity. This is not a non-Gaussian fixed point; the coupling vanishes at the fixed
point, so the theory reduces to pure GR in the UV. The structure---a coupling
that decreases monotonically toward zero at high probe scales---is analogous
in form to Yang--Mills theory in $d=4$, but $\km$ does not run
(Section~\ref{sec:rg}), so the mechanisms are distinct.
...
\begin{itemize}
  ...
  \item \CMSTG\ belongs to the general Horndeski class but acquires
    ...
    To be precise: the Galileon shift symmetry $\pi \to \pi + c + b_\mu x^\mu$
    is a symmetry of the Galileon action, and the associated Ward identities
    enforce the UV cancellations algebraically. The memory kernel is instead
    a postulate about the form of the propagator~\eqref{eq:propagator} in the
    regulated theory; it is not derivable from any symmetry of the
    action~\eqref{eq:action} and enters as a defining regulator rather than
    as a consequence of an algebraic structure of the Lagrangian.
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{$\Lz$ fixed point (SIM105).} The beta function
    $\beta(\Lz) = -\Lz^3\km^2/(16\pi^2 m_0^2)$ is negative for all $\Lz > 0$
    and $\km > 0$. The running coupling has a UV fixed point at $\Lz = 0$ (the
    GR limit) and no infrared Landau pole. The mechanism is that of a negative
    beta function driving $\Lz$ to its GR-limit fixed point: $\km$ is a fixed
    input of the theory rather than a running coupling, and the flow describes
    the probe-scale sensitivity of $\Lz$ below the memory scale.
...
\bibitem{Cheung2008}
  ...
  JHEP \textbf{0803}, 014 (2008), arXiv:0709.0293.
\bibitem{Gubitosi2013}
  G.~Gubitosi, F.~Piazza, and F.~Vernizzi,
  ``The Effective Field Theory of Dark Energy,''
  JCAP \textbf{02}, 032 (2013), arXiv:1210.0201.
\bibitem{Reuter1998}
  M.~Reuter,
...
\bibitem{MaggioreMC2014}
  M.~Maggiore and M.~Mancarella,
  ``Nonlocal gravity and dark energy,''
  Phys.\ Rev.\ D \textbf{90}, 023005 (2014), arXiv:1402.0448.
\bibitem{Belgacem2018}
  ...
  JCAP \textbf{03}, 002 (2018), arXiv:1712.07066.
