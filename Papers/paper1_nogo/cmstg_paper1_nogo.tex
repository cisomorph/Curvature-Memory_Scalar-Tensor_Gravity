...
\begin{abstract}
We establish two complementary structural no-go theorems that together prove the
Phase~1 canonical action of Curvature-Memory Scalar-Tensor Gravity (\CMSTG) is
the unique covariant scalar-tensor solution consistent with both CMB and DESI~Y1
BAO observations within its action class. \textbf{Theorem~1}: any scalar field
sourced by the Ricci scalar $R>0$ and initialized at zero grows monotonically,
increasing the effective gravitational coupling $\Feff$ and suppressing $H(z)$
at DESI redshifts---the wrong direction to reduce the $2.77\sigma$ tension.
\textbf{Theorem~2}: any mechanism that raises $H(z)$ in the interval
$z\in[0,z_{\rm rec}]$ while keeping the sound horizon $\rs$ fixed necessarily
shifts the CMB acoustic angle $\thetastar$ above the Planck bound at more
than $30\sigma$. Thirteen simulation-based mechanism tests (SIM131--SIM136,
SIM141--SIM143, and two structural arguments) verify both theorems exhaustively
across the identified Tier~2 mechanism space. The $2.77\sigma$ DESI~Y1 residual
is a structural prediction of Phase~1 canonical \CMSTG, not a defect; the
...
The Phase~1 canonical \CMSTG\ action,
\begin{equation}
...
It is UV-finite through two loops with an explicit asymptotic safety fixed
point at $\Lz=0$ (Paper~II of this series~\cite{WilsonPapers}). Its one
...
addressed by three successive simulation programmes (Phases 2--4, SIM112--SIM143),
summarised in Paper~IV of this series~\cite{WilsonPapers}.
...
\begin{proof}
Any increase in $H(z)$ at $z<z_{\rm rec}$ reduces the integrand
$c/H(z)$, decreasing $\DC$ for fixed $z_{\rm rec}$. With $\rs$ fixed (no
pre-recombination modification), $\thetastar = \rs/\DC$ increases. The
Planck constraint $100\thetastar=1.04101\pm0.00029$ has a fractional
precision of $0.03\%$. Resolving the DESI tension requires raising $H(z)$
by approximately $5\%$ over $z\in[0.5,1.3]$, which shifts $\DC$ by
$\sim1.5\%$ and hence $\thetastar$ by $\sim50\sigma$. The CMB and DESI
constraints are mutually exclusive for any late-time $H(z)$ boost with
fixed $\rs$. $\square$
\end{proof}
...
\subsubsection{SIM140 --- Step Potential $V(\Psi)$ (P4-C, deferred)}

SIM140 specifies a step potential keeping $\Psi$ frozen at $\Psibar=2.62\,\Mpl$
until $z_{\rm trans}\sim2$, then releasing it to roll to smaller $\Psi$. The
SIM138 diagnostic showed the DESI tension is distributed across $z\in[0.51,1.32]$,
requiring $H(z)$ enhancement across this range. A step at $z_{\rm trans}\sim2$
must raise $H(z)$ in exactly this range---making the $\thetastar$ violation
from Theorem~\ref{thm:nogo2} unavoidable. The $\rs$ loophole is absent by
SIM143. SIM140 was not executed; its verdict is predicted FAIL\_CMB by
structural argument.

\subsection{Completeness of the Phase~4 Search}

Table~\ref{tab:p4summary} lists all four Tier~2 mechanism classes.

\begin{table}[h!]
\centering
\caption{Phase~4 Tier~2 exhaustion. All four mechanism classes fail.}
\label{tab:p4summary}
\begin{tabular}{lllll}
\toprule
...
SIM143 & Bi-scalar $\Psi+\phi$        & P4-D & $1.053\sigma$       & FAIL/STRUCTURAL \\
SIM140 & Step potential $V(\Psi)$     & P4-C & (predicted)         & DEFERRED/STRUCTURAL \\
\bottomrule
\end{tabular}
\end{table}
...
the \CMSTG\ framework. \textbf{Phase~1 canonical \CMSTG\ is declared the final
theory within the class of covariant scalar-tensor extensions with retarded
memory kernels.} Its $2.77\sigma$ DESI tension is a structural prediction.
...
\bibitem{WilsonPapers}
  C.~R.~B.~Wilson,
  ``Curvature-Memory Scalar-Tensor Gravity: Papers I--VII,''
  \url{https://github.com/cisomorph/Curvature-Memory_Scalar-Tensor_Gravity} (2025--2026).
\bibitem{Abbott2017}
