...
\begin{abstract}
This is Paper~II of a four-paper series on Curvature-Memory Scalar-Tensor
Gravity (\CMSTG). We present the complete framework of \CMSTG: a covariant
scalar-tensor extension of general relativity in which the scalar field $\Psi$
encodes retarded curvature history through a non-minimal coupling $\Lz\Psi^2 R$
to the Ricci scalar. We derive the Euler--Lagrange and modified Friedmann
equations, establish the GR-recovery conditions, reduce the theory to the FLRW
cosmological background, and demonstrate canonical quantization at free-field level.

The Phase~1 canonical action with locked parameters $\Lz = 0.003\,\Mpl^{-2}$,
$\Psibar = 2.62\,\Mpl$, and effective gravitational coupling $\Feff = 0.521$ is
tested against nine precision datasets: BAO full-covariance refit
(BOSS+eBOSS+DESI~Y1), CMB TT/TE/EE power spectra (Planck plikHM), RSD growth
rate $f\sigma_8$, non-linear structure growth $\sigma_8$, gravitational wave
speed, Solar System PPN constraints, and dark energy equation of state. All
tests pass at current observational precision. The effective gravitational
coupling $\Geff/G = 1 - 15\,\mathrm{ppm}$ at $\Lz = 0.003$, placing \CMSTG\
observationally degenerate with $\Lambda$CDM at the $10^{-5}$ level; the
coupling is unconstrained by current data with an estimated detection threshold
at $\Lz \sim 0.05$.

The one residual is a $2.77\sigma$ tension with DESI~Y1 BAO. Paper~I of this
series~\cite{WilsonPaperI} establishes this as a structural floor, irreducible
within the \CMSTG\ action class by any mechanism in the identified Tier~2
mechanism space, including a completeness probe (SIM144) confirming that
mixed-source scalars coupling simultaneously to curvature and matter also fail. Paper~III~\cite{WilsonPaperIII} establishes the UV finiteness
of the theory in the resummed propagator. Paper~IV~\cite{WilsonPaperIV}
establishes that the Phase~1 canonical action cannot replace dark matter at
galactic scales without additional physics, defining a clear boundary on the
scope of the framework as it stands.
\end{abstract}
...
protection of the graviton sector, and a $\Lz$ fixed point at
...
the \CMSTG\ action class by any extension in the curvature-sourced scalar,
late-time $H(z)$, or mixed-source scalar mechanism space (SIM131--SIM144). Section~\ref{sec:discussion} places these
...
(Paper~III, SIM102).
The frame in which the kernel $e^{-k^2/k_m^2}$ is written---the cosmological
rest frame defined by CMB isotropy---and the scope and non-local EFT precedents
for this postulate are discussed in Paper~III, \S2.2(d)~\cite{WilsonPaperIII}.
...
\paragraph{Method (SIM90, SIM98).} Two complementary CMB analyses are reported.
SIM90 uses a Gaussian prior on the $(H_0, \Omega_m)$ marginal extracted from
the Planck~2018 TT+TE+EE+lowE+lensing parameter chains ($\sigma_{H_0} =
0.54$~km/s/Mpc, $\sigma_{\Omega_m} = 0.0073$, $\rho = -0.90$)~\cite{Planck2018}.
This is a parameter-level approximation to the CMB constraint and is appropriate
for rapid exploration of the $(H_0, \Omega_m, r_d, \Lz)$ parameter space; we use
it to identify the joint optimum. SIM98 then re-evaluates the \CMSTG\ model
against the actual Planck~2018 plikHM TTTEEE likelihood at the joint optimum,
providing the proper likelihood comparison. We report both: the parameter-prior
result (SIM90) shows that \CMSTG\ and $\Lambda$CDM converge to identical
$(H_0, \Omega_m)$ under the Planck constraint, and the likelihood result (SIM98)
shows that at the joint optimum, $\Delta(-2\ln\mathcal{L}) = -0.013$ between
\CMSTG\ and $\Lambda$CDM. The two analyses agree on the conclusion that \CMSTG\
is observationally degenerate with $\Lambda$CDM at current precision; SIM98 is
the methodologically definitive result and SIM90 is reported only because it is
the procedure used to locate the joint optimum.
SIM97 provides a diagnostic baseline by evaluating the same Planck~2018 plikHM
TTTEEE likelihood at the BAO-only best-fit parameters ($H_0 = 68.14$~km/s/Mpc,
$\Omega_m = 0.294$) rather than the joint CMB+BAO optimum; the resulting
$\Delta(-2\ln\mathcal{L}) = +7.0$ quantifies the CMB penalty incurred by using
BAO-only parameters and motivates the joint optimisation. SIM98 supersedes it
as the methodologically definitive likelihood comparison.
...
without violating CMB constraints, including mixed-source scalars with
simultaneous curvature and matter coupling (SIM144).
...
luminosity distance ratio $|d_L^{\rm GW}/d_L^{\rm EM} - 1|
= \tfrac{1}{2}|\Delta\ln M_{\rm eff}^2|$ integrated over $z\in[0,0.009]$;
since $\Psi$ is frozen at $\Psibar$ to $O(10^{-5})$ over this interval
(SIM108 background), this evaluates to $1.8\times10^{-11}$;
BBN: $\Geff(z_{\rm BBN})/G = 0.994$ (inside 10\% bound). All six GW tests pass.
...
\begin{equation}
  ...
  \qquad
  c_s^2 = \frac{c^2}{3(1 + R_b)}\,,\qquad
  R_b = \frac{3\rho_b}{4\rho_\gamma}\,.
  \label{eq:rd_definition}
\end{equation}

We integrate~\eqref{eq:rd_definition} with the full \CMSTG\ Friedmann equation
and the standard Planck-era treatment of relativistic species. The radiation
density includes the neutrino contribution at $N_{\rm eff} = 3.046$:
\begin{equation}
  \rho_r(z) = \rho_\gamma(z) \left[ 1 + \tfrac{7}{8}
  \left(\tfrac{4}{11}\right)^{4/3} N_{\rm eff} \right],
\end{equation}
with photon temperature $T_{\gamma,0} = 2.7255\,$K and helium fraction
$Y_{\rm He} = 0.245$ (BBN-consistent). The drag-epoch redshift
$z_{\rm drag} = 1059.94$ is taken from the Planck~2018 best-fit
calibration~\cite{Planck2018}.

\paragraph{$\Lambda$CDM baseline.}
At $\Lz = 0$, the integration yields
\begin{equation}
  r_d^{\Lambda{\rm CDM}} = 146.785\,\mathrm{Mpc},
\end{equation}
in agreement with the Planck~2018 measurement $r_d = 147.09 \pm 0.26\,\mathrm{Mpc}$
to within $0.21\%$. This validates the integration pipeline against the standard
result. The 0.21\% (2080\,ppm) offset is dominated by the simplified single-fluid
Friedmann equation: evaluated at the Planck~2018 best-fit cosmology
($H_0 = 67.36$, $\Omega_m = 0.315$), the same pipeline gives
$r_d = 146.659\,\mathrm{Mpc}$, a 2940\,ppm deficit from the Planck reference.
The SIM90 parameters ($H_0 = 67.59$, $\Omega_m = 0.312$) yield a 860\,ppm
higher baseline within the same simplified pipeline, partially offsetting the
Friedmann deficit to give the 2080\,ppm net offset. Both contributions are
common-mode between \CMSTG\ and $\Lambda$CDM and therefore cancel exactly in
the fractional shift $\Delta r_d/r_d$ reported below.

\paragraph{\CMSTG\ correction.}
At the Phase~1 canonical locked action ($\Lz = 0.003\,\Mpl^{-2}$,
$\Psibar = 2.62\,\Mpl$), the same integration yields
\begin{equation}
  r_d^{\CMSTG} = 146.785\,\mathrm{Mpc}\,,
  \qquad
  \frac{\Delta r_d}{r_d} = -0.68\,\mathrm{ppm}\,.
\end{equation}
Across the cosmologically allowed range $\Lz \in [0, 0.095]$ established by
the joint CMB+BAO fit (SIM90), the maximum fractional shift is
\begin{equation}
  \left|\frac{\Delta r_d}{r_d}\right|_{\max}
  = 21.5\,\mathrm{ppm}
  \quad\text{at } \Lz = 0.095\,,
\end{equation}
to be compared with current BAO measurement precision of $\sim 0.3\%$
($\sim 3000\,$ppm). The \CMSTG\ and $\Lambda$CDM predictions for $r_d$ are
therefore degenerate at all observationally allowed couplings; a detection of
\CMSTG\ through $r_d$ alone would require $\Lz \gg 0.1$, well above the
joint CMB+BAO upper bound.

The physical reason for the small shift is that the scalar energy density at the
drag epoch is $\rho_\Psi/\rho_{\rm tot} \sim 10^{-10}$: the field $\Psi$ is
dynamically irrelevant during recombination, and \CMSTG\ shares essentially
identical pre-recombination physics with $\Lambda$CDM. The difference between
the two theories is concentrated at low redshift, which is the regime tested by
the BAO and DESI~Y1 analyses of Sections~\ref{sec:bao} and~\ref{sec:desi}.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.75\textwidth]{figures/fig9_sound_horizon.pdf}
  \caption{Sound horizon $r_d$ as a function of $\Lz$ (SIM101), computed with
  $N_{\rm eff} = 3.046$, $T_{\gamma,0} = 2.7255\,$K, and $z_{\rm drag} = 1059.94$.
  Left: $r_d(\Lz)$ with the \CMSTG\ result (blue) and $\Lambda$CDM baseline
  (dashed). Centre: fractional shift $\Delta r_d/r_d$ in ppm; the maximum is
  $21.5\,\mathrm{ppm}$ at $\Lz = 0.095$. Right: scalar energy fraction
  $\rho_\Psi/\rho_{\rm tot}$ at the drag epoch ($\sim 10^{-10}$ throughout
  the allowed range), confirming dynamical irrelevance of $\Psi$ at
  recombination. Current BAO precision is $\sim 3000\,\mathrm{ppm}$.}
  \label{fig:rd}
\end{figure}
...
The Phase~1 locked action defines the observationally preferred parameter
combination in the $\Lz\Psi^2 R$ coupling class: one that simultaneously
satisfies (i) $\Geff > 0$ everywhere, (ii) $c_T = c$ exactly,
(iii) $|\gamma-1| < 2.3\times10^{-5}$,
(iv) $\chi^2_{\rm CMB+BAO} \leq \chi^2_{\Lambda\rm CDM} + 0.5$, and
(v) $|\Delta r_d/r_d| < 10^{-3}$. It is not a sharply isolated point: the
best-fit values $\Lz = 0.003$, $\Psibar = 2.62\,\Mpl$ emerge from the joint
CMB+BAO optimisation (SIM90), but $\Lz = 0.013$ lies on the same landscape
with $\Delta\chi^2 < 0.004$, establishing a flat degeneracy valley extending
to $\Lz \lesssim 0.095$ (95\% confidence, SIM91). The observational constraint
is that $\Lz$ is small, not that it is precisely $0.003$. The detection
threshold $\Lz \sim 0.05$ marks the boundary above which future surveys would
distinguish \CMSTG\ from $\Lambda$CDM.
...
\begin{enumerate}
  ...
  \item \textbf{DESI residual:} The $2.77\sigma$ DESI~Y1 BAO tension in the
        ...
        including mixed-source scalars confirmed by SIM144.
