...
\title{\textbf{UV Completeness and Asymptotic Freedom of\\
...
\begin{abstract}
We establish the UV structure of Curvature-Memory Scalar-Tensor Gravity
(\CMSTG) at one loop and two loops through four numerical simulation programmes
(SIM102, SIM104, SIM105, SIM106). The one-loop $\Psi$ self-energy in the bare
theory diverges quartically; the memory kernel $e^{-2k^2/k_m^2}$ regulates
this to the finite result $\Sigma(0) = \Lz^2 k_m^4/(64\pi^2)$, confirmed
numerically at better than 0.22\% across two orders of magnitude in $k_m$.
We compute all four 1PI diagrams at one loop: (A) the $\Psi$ self-energy,
regulated by the memory kernel; (B) the graviton wavefunction renormalization,
protected by diffeomorphism Ward identity $\Pih(0) = 0$; (C) the vertex
correction $\delta\Lz/\Lz$; and (D) the two-loop $\Psi$ self-energy. The
three contributions form a hierarchical expansion with magnitudes $1$,
$2.4\times10^{-2}$, and $3.4\times10^{-6}$ at the naturalness threshold
$\knat = 9.15\,\mathrm{Mpc}^{-1}$, confirming a stable perturbative expansion.
The renormalization-group flow of $\Lz$ is negative at all scales:
$\beta(\Lz, \km) = -\Lz^3 \km^2/(16\pi^2 m_0^2)$, establishing asymptotic
freedom with UV fixed point $\Lz \to 0$ (the GR limit) and no IR Landau pole.
At two loops, the graviton sector is UV-finite via three independent mechanisms:
the Ward identity, Dyson resummation of the $\Psi$ propagator, and the
finiteness of $\partial^2\Pi_{hh}/\partial p^4$ in $d = 4$. The graviton
wavefunction renormalization $\Delta Z_h/Z_h \approx 1.4\times10^{-6}$ is
negligible. Together these results establish that \CMSTG\ is UV-finite at
full one-loop in the resummed propagator and UV-complete at two loops in the
graviton sector, with an explicit RG trajectory connecting the observed
$\Lz_{\rm obs} = 0.003\,\Mpl^{-2}$ to the GR fixed point.
\end{abstract}

\keywords{scalar-tensor gravity; UV completeness; asymptotic freedom; memory kernel; renormalization group; loop corrections}
...
progress: asymptotic safety~\cite{Reuter1998,Reuter2012}, which seeks a
non-Gaussian UV fixed point in the full theory space of metric gravity; and
the Horndeski quantization programme~\cite{Horndeski1974,deRham2011,Gleyzes2015},
which analyses specific scalar-tensor actions for renormalizability and
stability. \CMSTG\ belongs to neither programme directly: it is a covariant
scalar-tensor theory in which the scalar $\Psi$ acquires a non-minimal
curvature coupling $\Lz\Psi^2 R$ and a non-local memory kernel that softens
its UV propagator~\cite{WilsonPaperII}. This paper (Paper~III of this series)
establishes where \CMSTG\ sits in the UV landscape; the companion
paper~\cite{WilsonPaperII} (Paper~II) derives the framework and cosmological
...
\begin{equation}
  ...
  \label{eq:action}
\end{equation}
with locked parameters $\Lz = 0.003\,\Mpl^{-2}$, $m_0 = 0.01\,\mathrm{Mpc}^{-1}$,
and cosmological background value $\Psibar = 2.62\,\Mpl$. The effective
gravitational coupling is $G_{\rm eff} = G/(1 + 2\Lz\Psibar^2)$; at the
best-fit $G_{\rm eff}/G = 1 - 16\,\mathrm{ppm}$.

The scalar propagator in momentum space is
\begin{equation}
  \Delta_\Psi(k) = \frac{e^{-k^2/k_m^2}}{k^2 + m_0^2}\,,
  \label{eq:propagator}
\end{equation}
where $\km$ is the memory scale. Equation~\eqref{eq:propagator} is the
resummed propagator obtained by Dyson-summing the memory self-energy
insertions; it reduces to the standard massive propagator as $\km \to \infty$.
The exponential damping converts all loop integrals from power-law divergent
to finite once $\km < \infty$.

This paper is organized as follows. Section~\ref{sec:oneloop-psi} presents the
one-loop $\Psi$ self-energy (SIM102). Section~\ref{sec:diagrams} covers the
complete four-diagram one-loop structure (SIM104). Section~\ref{sec:ward}
establishes the Ward identity protection. Section~\ref{sec:rg} derives the
RG flow and asymptotic freedom (SIM105). Section~\ref{sec:twoloop} covers the
two-loop graviton sector (SIM106). Section~\ref{sec:comparison} places the
results in the context of asymptotic safety and the Horndeski programme.
...
\subsection{Implications for UV Completeness}
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Renormalization Group Flow and Asymptotic Freedom}
\label{sec:rg}
...
\begin{equation}
  ...
  \label{eq:beta}
\end{equation}
The beta function is \emph{negative} for all $\Lz > 0$, $\km > 0$. This is
the hallmark of asymptotic freedom: the coupling weakens as the UV scale
increases.
...
\subsection{Conclusion from SIM106}

All three two-loop graviton diagrams are UV-finite. The three mechanisms
responsible are: (1)~the Ward identity $\Pih(0) = 0$ exact at all loop
orders; (2)~the resummed $\Psi$ propagator regulates $\Psi$-bubble
insertions; (3)~$dI/dp^2|_0$ is finite in $d=4$ without memory regulation.
...
\CMSTG\ is UV-complete through two loops in the graviton sector.
...
\begin{itemize}
  ...
  \item \CMSTG\ belongs to the general Horndeski class but acquires
    ...
    not imposed by a symmetry of the action; it emerges from the Dyson
    resummation of an infinite class of memory self-energy insertions.
\end{itemize}
...
UV completeness depends on the memory scale $\km$ remaining finite:
taking $\km \to \infty$ recovers the bare quartic divergence. The theory
is UV-complete as a regulated QFT with a physical cutoff at $\km$, not as
a standard perturbatively renormalizable theory.
...
We have established the UV structure of \CMSTG\ at one loop and two loops
through four independent simulation programmes:

\begin{enumerate}
  \item \textbf{SIM102}: The one-loop $\Psi$ self-energy is quartically
    divergent in the bare theory and exactly regulated by the memory kernel
    to $\Sigma(0) = \Lz^2\km^4/(64\pi^2)$. Agreement with the analytic
    formula is better than 0.22\% across two orders of magnitude in $\km$.

  \item \textbf{SIM104}: All four 1PI one-loop diagrams are accounted for.
    The graviton mass $\Pi_{hh}(0) = 0$ is protected by the Ward identity.
    The perturbative hierarchy at the naturalness scale is $1$, $2.4\times10^{-2}$,
    $3.4\times10^{-6}$, confirming a convergent loop expansion.

  \item \textbf{SIM105}: The beta function $\beta = -\Lz^3\km^2/(16\pi^2 m_0^2)$
    is negative, establishing asymptotic freedom. The running coupling satisfies
    $1/\Lz^2(\km) = 1/\Lz_{\rm obs}^2 + (\km^2 - \knat^2)/(16\pi^2 m_0^2)$
    with a UV fixed point at $\Lz = 0$ and no IR Landau pole.

  \item \textbf{SIM106}: All two-loop graviton sector diagrams are UV-finite.
    The graviton wavefunction renormalization $\Delta Z_h/Z_h \approx 1.4\times10^{-6}$
    is negligible. \CMSTG\ is UV-complete through two loops in the graviton sector.
\end{enumerate}
...
cancellations of the Galileon. It is instead a combination of (i)~memory
damping, which converts power-law divergences into finite functions of
$\km$; (ii)~the Ward identity, which protects the graviton mass at all
loop orders; and (iii)~asymptotic freedom, which guarantees $\Lz \to 0$
in the deep UV.

The open question is whether the memory scale $\km$ itself is determined
by a deeper principle or remains a free parameter. The naturalness constraint
$\km < 9.15\,\mathrm{Mpc}^{-1}$ is consistent with current observational
bounds, but a first-principles derivation of $\km$ from the full non-local
...
(simulations SIM102, SIM104, SIM105, SIM106).
...
\bibitem{WilsonPaperII}
  C.~R.~B.~Wilson,
  ``Curvature-Memory Scalar-Tensor Gravity: Framework and Cosmological Consistency''
