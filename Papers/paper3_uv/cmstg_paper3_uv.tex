...
\begin{abstract}
We establish the ultraviolet structure of Curvature-Memory Scalar-Tensor
Gravity (\CMSTG) at one and two loops in the resummed propagator. The theory
...
specifies an effective field theory. With this input, four numerical simulation
programmes (SIM102, SIM104, SIM105, SIM106) establish the following.
...
This is Paper~III of a four-paper series; companion papers establish
(I)~the no-go theorems~\cite{WilsonPaperI}, (II)~the framework and
observational consistency~\cite{WilsonPaperII}, and (IV)~galactic-scale
...
nor that the kernel is uniquely selected by a deeper symmetry principle. We
claim only the following: given the action~\eqref{eq:action} and the
kernel~\eqref{eq:propagator}, the resulting quantum theory is finite to all
orders explicitly computed (one and two loops in the graviton sector), the
renormalization-group flow of $\Lz$ at fixed $\km$ has a fixed point at
$\Lz = 0$, and the qualitative UV behaviour is expected to be insensitive to
the precise functional form of the kernel provided it falls off at least as fast
as a Gaussian at high $k$. A systematic verification across kernel forms is
left for future work.
