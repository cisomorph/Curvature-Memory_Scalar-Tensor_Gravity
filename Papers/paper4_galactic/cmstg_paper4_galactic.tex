...
\begin{abstract}
We establish systematic galactic-scale constraints on Curvature-Memory
Scalar-Tensor Gravity (\CMSTG) through three independent simulation-based
tests (SIM99, SIM100, SIM103) targeting NGC~3198 and NGC~6503, the
best-observed spiral galaxies with flat rotation curves. The Phase~1 canonical
locked action fails to produce flat rotation curves through any of the three
candidate mechanisms: (1) the static Yukawa field profile is exponentially
steeper than the $\rho \propto r^{-2}$ isothermal halo required for curve
flattening (SIM99); (2) the time-domain 1+1D wave equation confirms the static
result and shows that the curvature coupling $2\Lz|R_{\rm gal}|$ is
$\sim2\times10^6$ times weaker than the field mass at galactic densities (using
the simulation test mass $m_0 = 0.1\,\mathrm{kpc}^{-1}$; even weaker at the
locked cosmological $m_0$), making condensation impossible (SIM100); (3) Vainshtein-type non-linear kinetic
screening fails by six orders of magnitude, with the Vainshtein ratio
$\mathcal{V} \leq 3.7\times10^{-6}$ across the full scanned parameter space
(SIM103). The locked action energy density $\rho_\Psi = 1.6\times10^{-3}$
$M_\odot\,\mathrm{kpc}^{-3}$ falls $2.7\times10^6$ short of the isothermal
requirement at $r = 10$~kpc. These constraints are clean and model-independent:
every identified mechanism within the \CMSTG\ action class has been exhausted.
A first-principles coupling between $\Psi$ and baryonic matter is the minimum
new physics required for galactic-scale predictions.
\end{abstract}
...
action class. The result is not a fine-tuning problem or a quantitative
...
at typical galactic densities. At the locked cosmological value
$m_0 = 6.88\times10^{-8}\,\mathrm{kpc}^{-1}$, this ratio is
$\approx 3.6\times10^{-8}$---even weaker, confirming the locked-action
failure is more extreme than the simulation constraint. This single ratio
determines the failure of all three mechanisms.
...
(simulation test mass $m_0 = 0.1\,\mathrm{kpc}^{-1}$; the locked cosmological
$m_0$ gives an even smaller ratio, $\approx 3.6\times10^{-8}$). This
makes the scalar field cosmological (Compton wavelength $\gg$ galactic
size) and prevents any galactic-scale field gradient from building up
to isothermal amplitudes.
...
\begin{enumerate}
  ...
  \item \textbf{Time-domain dynamics (SIM100):} The curvature coupling
    ...
    all galactic densities (simulation test mass; locked $m_0$ is weaker still). Condensation requires neutron-star densities.
...
at the simulation test mass, even smaller at the locked cosmological $m_0$)
rather than a parameter choice. A first-principles coupling between $\Psi$ and baryonic
