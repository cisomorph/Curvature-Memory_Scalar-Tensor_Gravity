...
A subsequent $\Lz$ sweep over eight decades (SIM150) establishes that the
$\Geff/G_N$ ceiling is structural and parameter-universal: the only value
satisfying $\Geff/G_N \geq 3.115$ is $\Lambda_{0,{\rm req}} = 0.154$ (51 times the
locked value), which violates the Cassini Solar-System PPN bound by
$\sim2\times10^4\times$ and all four remaining locked cosmological constraints
simultaneously. No $\Lz$ within the Solar-System-consistent range of the locked
action can revive the galactic mechanism.
...
mechanisms accessible within the action class. Four independent
...
\begin{enumerate}
  ...
  \item \textbf{Memory-kernel $\Geff$ boost (SIM149--SIM150):} The
    ...
    ($\Lambda_{0,{\rm req}} = 0.154$) violates the Cassini PPN bound by
    $\sim2\times10^4\times$ and all five remaining locked constraints.
\end{enumerate}

The energy density deficit at $r = 10\,\mathrm{kpc}$ is
$2.7\times10^6\times$ under all four mechanisms. A direct matter coupling
$\beta \sim 1{,}600\,\Lz$ would supply the missing density but is excluded
by cosmological constraints (Paper~II~\cite{WilsonPaperII}). The
$\Geff/G_N$ ceiling is structural: it is built into the form of $F(\Psi)
= \tfrac{1}{2}+\Lz\Psi^2$ and the requirement $|\Geff/G_N - 1| < 10^{-5}$
on cosmological scales, which jointly fix $\Lz\Psi_0^2 \ll 1$ and hence
the ceiling at $1 + 2\Lz\Psi_0^2 \approx 1 + O(10^{-2})$.
...
(simulations SIM99, SIM100, SIM103, SIM149, SIM150).
