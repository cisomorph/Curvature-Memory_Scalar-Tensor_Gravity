...
Observational Constraints from BAO, CMB,\\
...
We subject RIFT to twenty-two independent observational and theoretical tests (Sims~87--110).
The key results are:
...
(xix)~Solar System viability (Sim~107): $|\gamma-1|=1.78\times10^{-5}$ satisfies
    the Cassini bound; chameleon screening ensures GR recovery inside compact bodies;
(xx)~gravitational wave constraints (Sim~108): 6/6 PASS---$c_T=c$ exactly
    ($G_5=0$), $m_g=0$ (Ward identity), GW170817 luminosity distance deviation
    $1.8\times10^{-11}$, $G_\mathrm{eff}$ at BBN offset $0.60\%$ ($<10\%$ bound);
(xxi)~dark energy equation of state (Sim~109): $w_0=-0.992$, $w_a=-0.082$;
    consistent with Planck at $1.3\sigma$, in $3.6\sigma$ tension with DESI Y1
    $w_0$--$w_a$; dynamical $\Psi$ ($m_0\gtrsim H_0$) needed to resolve DESI tension;
(xxii)~inflationary viability (Sim~110): locked action fails slow-roll inflation
    ($n_s\approx0.932$, $>5\sigma$ from Planck); Starobinsky attractor accessible
    with UV non-minimal coupling $\xi\sim10^4$, testable by CMB-S4/LiteBIRD.
In all cosmological tests (Sims~87--98 and 107--109), RIFT at the joint best-fit is
statistically indistinguishable from $\Lambda$CDM at current precision. The recursive field
...
(DESI~Y5, Euclid, CMB-S4, LiteBIRD).
\end{abstract}
...
% ============================================================
\section{Solar System Viability: Effective Matter Coupling (Sim~107)}
\label{sec:sim107}

\subsection{Motivation}

Passing CMB and BAO tests is necessary but not sufficient for a modified-gravity
theory.  The $\Psi$ field mediates a long-range scalar force that must not
violate Solar System constraints.  We derive the $\Psi$--matter coupling from
the linearised RIFT field equation and test against the Cassini bound
($|\gamma-1|<2.3\times10^{-5}$, Bertotti et al.\ 2003).

\subsection{Effective Coupling}

Linearising around background $\Psi_0$ in the presence of matter perturbation
$\delta\rho$:
\[
  \Box\,\delta\Psi - m_{\rm eff}^2(\rho)\,\delta\Psi
  = 16\pi G_N \Lambda_0\Psi_0\,\delta\rho ,
  \qquad
  m_{\rm eff}^2(\rho) = m_0^2 + 2\Lambda_0\rho .
\]
In scalar-tensor language the dimensionless coupling is
\[
...
The effective mass is \emph{chameleon-like}: $m_{\rm eff}$ increases with
density, screening the field inside compact bodies.  At Solar interior density
($\rho_\odot\approx1400\,\mathrm{kg\,m^{-3}}$) the Compton wavelength drops to
$\lambda_\Psi\approx8\times10^{-11}$\,Mpc (sub-femtometre), providing complete
screening.  In interplanetary space $\lambda_\Psi\sim4\times10^6$\,Mpc, so
the force is unscreened but couples only as $\alpha^2\sim\Lambda_0^2\Psi_0^2$.

\subsection{Results}

At the natural background value $\Psi_0 = 1\,M_{\rm Pl}$:
\begin{align*}
  \alpha^2 &= 8.9\times10^{-6}, \\
  |\gamma-1| &= 1.78\times10^{-5} \quad < \quad 2.3\times10^{-5}\ \text{(Cassini)}.
\end{align*}
The critical constraint is $|\Psi_0|<1.14\,M_{\rm Pl}$; the locked action
\emph{requires no additional screening mechanism}.
Table~\ref{tab:sim107} summarises the environmental chameleon scan.

\begin{table}[h]
\centering
\caption{Effective $\Psi$ mass and fifth-force Compton wavelength (Sim~107).}
\label{tab:sim107}
\begin{tabular}{lccc}
\hline
Environment & $m_{\rm eff}/H_0$ & $\lambda_\Psi$ [Mpc] & Chameleon \\
\hline
Cosmic mean   & $7.5\times10^{-2}$  & $5.7\times10^{4}$ & yes \\
Interstellar  & $13.8$              & $310$             & yes \\
...
\paragraph{Verdict.}  RIFT passes Solar System viability for all
$|\Psi_0|\lesssim 1\,M_{\rm Pl}$.  The coupling is naturally suppressed by
$\alpha^2=O(\Lambda_0^2)\sim10^{-5}$; no fine-tuning is required.

% ============================================================
\section{Gravitational Wave Speed, GW170817, and BBN (Sim~108)}
\label{sec:sim108}

\subsection{GW speed from the tensor perturbation equation}

The transverse-traceless tensor perturbation of the RIFT action obeys
\begin{equation}
  G_4(t)\!\left[\ddot{h} + \!\left(2H + \tfrac{\dot{G}_4}{G_4}\right)\dot{h}
  + \tfrac{k^2}{a^2}h\right] = 0,
  \quad G_4 = \tfrac{1}{16\pi G_N} + \Lambda_0\Psi_0^2.
\end{equation}
The gravitational wave speed $c_T^2 = 1$ \emph{exactly}: RIFT belongs to
the Horndeski class with $G_5=0$, and $G_5\ne0$ is the only source of
$c_T\ne c$ in that class.
...
\begin{table}[h]
\centering
\caption{SIM108 gravitational wave tests.}
\label{tab:sim108}
\begin{tabular}{lllc}
\hline
Test & RIFT prediction & Bound & Status \\
\hline
$|c_T/c-1|$                        & $0$ (exact)          & $<7\times10^{-16}$ & PASS \\
Graviton mass $m_g$                & $0$ (Ward identity)  & $<1.27\times10^{-23}$\,eV & PASS \\
$|d_L^{\rm GW}/d_L^{\rm EM}-1|$   & $1.8\times10^{-11}$  & $<0.06$ & PASS \\
$|G_{\rm eff}(z_{\rm BBN})/G_N-1|$& $0.60\%$             & $<10\%$ & PASS \\
Chirp mass bias $\delta\mathcal{M}/\mathcal{M}$ & $0.36\%$ & $<5\%$ & PASS \\
$P_T$ modification                 & $2\times10^{-9}$     & n/a & PASS \\
\hline
\end{tabular}
\end{table}

The $G_{\rm eff}$ constant offset of $-0.60\%$ arises from
$G_{\rm eff}=G_N/(1+2\Lambda_0\Psi_0^2)$ at the observationally constrained
$\Lambda_0=0.003$, $\Psi_0\sim1$; this is stable across all redshifts because
the slow-roll approximation gives $\Psi_0(z)\approx\Psi_0(0)$ for
$m_0=0.001\,H_0$ throughout the observable universe.

% ============================================================
\section{Dark Energy Equation of State (Sim~109)}
\label{sec:sim109}

The full RIFT background (modified Friedmann + Klein-Gordon) is integrated
numerically from $z=0$ to $z=5$.  The effective equation of state extracted
from $\rho_{\rm DE}(z)=3H^2-\rho_m-\rho_r$ and fitted to CPL gives:
\[
...
\begin{table}[h]
\centering
\caption{RIFT dark energy equation of state vs observations (Sim~109).}
\begin{tabular}{lccc}
\hline
Dataset & $w_0$ & $w_a$ & Tension \\
\hline
RIFT (locked) & $-0.992$ & $-0.082$ & --- \\
Planck 2018   & $-1.03\pm0.03$ & $0$ (fixed) & $1.3\sigma$ \\
DESI Y1 2024  & $-0.827\pm0.060$ & $-0.75\pm0.29$ & $3.6\sigma$ \\
\hline
\end{tabular}
\end{table}

The $\Psi$ field varies by only 11\% over $z\in[0,5]$ (slow roll, $m_0=0.001\,H_0$);
DE is cosmological-constant-dominated and $w\approx-1$.  RIFT passes Planck
at 1.3$\sigma$ but is in 3.6$\sigma$ tension with the DESI Y1 $w_0$-$w_a$ hint.

% ============================================================
\section{$m_0$ Scan: DESI Tension is Structural (Sim~111)}
\label{sec:sim111}

To test whether a larger $m_0$ can resolve the DESI tension, we scan
$m_0\in[0.001,10]\,H_0$ using the Sim~109 background solver.  The full
results are given in Section~\ref{sec:sim109} of the master paper; the
key finding is:
\begin{itemize}
  \item The minimum achievable DESI tension (Planck-consistent) is
    $\mathbf{3.44\sigma}$ at $m_0\approx0.31\,H_0$.
  \item For all Planck-consistent $m_0$, the CPL parameters satisfy
    $w_0\in[-1.002,-0.986]$ and $w_a\in[-0.114,-0.015]$, far from the DESI
    target ($w_0=-0.827$, $w_a=-0.75$).
  \item Above $m_0\approx 0.5\,H_0$, $\Psi$ evolves dramatically and
    violates Planck; above $m_0\gtrsim 1\,H_0$ the field is
    oscillation-dominated ($w\to 0$).
\end{itemize}
The obstruction is structural: $\rho_\Lambda$ dominates the DE budget
at all $m_0$ where $\Psi$ remains subdominant.  The locked RIFT action
\emph{cannot} produce the DESI $w_0$--$w_a$ hint; resolution requires
unlocking the action (removing the bare $\Lambda$ or adding a
$\Psi$--DE coupling).
...
The Einstein-frame potential $V_E=m_0^2\Psi^2/[2(1+2\Lambda_0\Psi^2)^2]$ has a maximum
at $\Psi_\mathrm{max}=1/\sqrt{2\Lambda_0}\approx12.9\,M_\mathrm{Pl}$.
Slow-roll inflation from the hilltop gives $N_\mathrm{max}\approx488\gg60$ and
$r\approx0.014$ (both PASS), but the spectral index $n_s\approx0.932$ fails at
$>5\sigma$.  The quartic extension $V_J=\lambda\Psi^4$ with $\Lambda_0=0.003$ also
fails ($n_s\approx0.931$, CMB window in transition zone).  The Starobinsky attractor
($n_s=0.967$, $r=0.003$) requires an independent UV coupling $\xi\sim10^4$,
testable by CMB-S4 and LiteBIRD.

\begin{table}[h]
\centering
\caption{Inflationary predictions vs Planck 2018 (Sim~110).}
\begin{tabular}{lccl}
\hline
Model & $n_s$ & $r$ & Status \\
\hline
RIFT quadratic, $\Lambda_0=0.003$ & $0.932$ & $0.014$ & FAIL ($n_s>5\sigma$) \\
RIFT quartic, $\Lambda_0=0.003$   & $0.931$ & $0.120$ & FAIL (transition zone) \\
Quartic $+$ $\xi\sim10^4$ (UV)    & $0.967$ & $0.003$ & PASS \\
\hline
\multicolumn{4}{l}{Planck 2018: $n_s=0.9649\pm0.0044$, $r<0.036$} \\
\hline
\end{tabular}
\end{table}
...
\begin{itemize}
  ...
  \item Solar System viability (Sim~107): PASS.  Linearised $\Psi$--matter coupling gives $|\gamma-1|=1.78\times10^{-5}$, satisfying the Cassini bound ($<2.3\times10^{-5}$).  Chameleon screening ($\lambda_\Psi\sim8\times10^{-11}$\,Mpc inside the Sun) provides complete GR recovery in dense environments; no fine-tuning required for $|\Psi_0|\lesssim1\,M_\mathrm{Pl}$.
  \item Gravitational wave constraints (Sim~108): 6/6 PASS.  $c_T=c$ exactly ($G_5=0$); $m_g=0$ (Ward identity); luminosity distance ratio $|d_L^\mathrm{GW}/d_L^\mathrm{EM}-1|=1.8\times10^{-11}$ ($\ll6\%$ GW170817 bound); $G_\mathrm{eff}$ at BBN offset $0.60\%$ ($<10\%$ bound); chirp mass bias $0.36\%$ ($<5\%$).
  \item Dark energy equation of state (Sim~109): $w_0=-0.992$, $w_a=-0.082$.  Consistent with Planck 2018 at $1.3\sigma$; in $3.6\sigma$ tension with the DESI Y1 $w_0$-$w_a$ hint.  Resolving DESI requires dynamical $\Psi$ ($m_0\gtrsim H_0$), testable by DESI Y5 and Euclid.
  \item Inflationary viability (Sim~110): the locked action fails slow-roll inflation.  The hilltop potential gives $n_s\approx0.932$ ($>5\sigma$ below Planck $0.9649\pm0.0044$) despite $N\approx488\gg60$ and $r\approx0.014$ (PASS).  The Starobinsky attractor ($n_s=0.967$, $r=0.003$) is accessible with an independent UV non-minimal coupling $\xi\sim10^4$, testable by CMB-S4 and LiteBIRD.
  \item DESI tension scan (Sim~111): full $m_0$ scan confirms the locked action cannot resolve the DESI $w_0$-$w_a$ hint.  Minimum achievable tension is $3.44\sigma$ at the best-fit $m_0$; the tension floor is structural, not a parameter optimisation failure.  Unlocking the action (Phase 2) is required.
\end{itemize}
