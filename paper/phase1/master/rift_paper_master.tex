...
and Twenty-Two Observational Tests\\[4pt]
\large\textit{Master Compilation: Papers~I--V}}
...
We subject RIFT to twenty-two independent observational and theoretical tests (Sims~87--110); numerical self-consistency is verified separately in Sims~82--86.
The key results are:
...
(xxii)~$m_0$ scan (Sim~111): full scan $m_0\in[0.001,10]\,H_0$ confirms locked
    action cannot resolve DESI tension; minimum achievable tension (Planck-consistent)
    $= 3.44\sigma$ at $m_0=0.31\,H_0$; structural obstruction --- $\rho_\Lambda$
    dominates DE at all Planck-consistent $m_0$, giving $w\approx-1$ universally;
    resolution requires unlocking the action (new $\Psi$--DE coupling or removal
    of bare $\Lambda$), testable by DESI~Y5/Euclid;
(xxiii)~inflationary viability (Sim~110): locked action fails slow-roll inflation ---
...
In all cosmological tests (Sims~87--98 and 107--111), RIFT at the joint best-fit is
statistically indistinguishable from $\Lambda$CDM at current precision. The recursive
field coupling $\Lambda_0$ is constrained to $\Lambda_0 < 0.095$ (95\% CL) and is
...
This is the \emph{master compilation} of the RIFT series, incorporating
...
\begin{table}[h]
\centering
\caption{UV completeness of RIFT through two loops (Sims~102--106).}
...
% ============================================================
\section{Dynamical $\Psi$: $m_0$ Scan for DESI Tension Resolution (Sim~111)}
\label{sec:sim111}

\subsection{Motivation}

Sim~109 found that the locked RIFT action with $m_0=0.001\,H_0$ predicts
$w_0=-0.992$, $w_a=-0.082$, in 3.6$\sigma$ tension with the DESI Y1
$w_0$--$w_a$ hint ($w_0=-0.827\pm0.060$, $w_a=-0.75\pm0.29$).
The natural question is whether increasing $m_0$ toward or above $H_0$
(making $\Psi$ dynamical) can reduce this tension while remaining Planck-consistent.

\subsection{Setup}

We scan $m_0\in\{0.001,0.003,0.01,0.03,0.1,0.2,0.3,0.5,0.7,1,1.5,2,3,5,10\}\,H_0$
using the same background solver as Sim~109 (DOP853, $\mathrm{rtol}=10^{-9}$),
with initial condition $\Psi(z=0)=1.0\,M_\mathrm{Pl}$ and the generalised
slow-roll velocity $y_0 = (2\Lambda_0\cdot12 - m_0^2)\,\Psi_0/3$, capped at $|y_0|\leq5$
for large $m_0$.  A fine scan of 30 points is then run around the coarse-grid minimum.
DESI tension is quantified as $\sigma_\mathrm{DESI}=\sqrt{\Delta w_0^2/\sigma_{w_0}^2+\Delta w_a^2/\sigma_{w_a}^2}$.

\subsection{Results}

\begin{table}[h]
\centering
\caption{$m_0$ scan: CPL parameters and tension (representative Planck-consistent values).}
\begin{tabular}{rcccc}
\hline
$m_0/H_0$ & $w_0$ & $w_a$ & $\sigma_\mathrm{DESI}$ & Planck \\
\hline
$0.001$ & $-0.992$ & $-0.082$ & $3.6$ & PASS \\
$0.10$  & $-0.991$ & $-0.088$ & $3.6$ & PASS \\
$0.20$  & $-0.988$ & $-0.103$ & $3.5$ & PASS \\
$0.30$  & $-0.986$ & $-0.114$ & $3.4$ & PASS \\
$0.50$  & $-1.002$ & $-0.015$ & $3.9$ & PASS \\
$0.70$  & $-1.066$ & $+0.401$ & $5.6$ & PASS \\
$1.00$  & $-1.204$ & $+1.544$ & $10.1$ & FAIL \\
$2.00$  & $+0.14$  & $-0.51$  & $16.1$ & FAIL \\
\hline
\end{tabular}
\label{tab:sim111}
\end{table}

The minimum achievable DESI tension among all Planck-consistent results
is $\mathbf{3.44\sigma}$ at $m_0\approx0.31\,H_0$ (fine scan).  This is
a negligible improvement over Sim~109's 3.6$\sigma$.
Above $m_0\approx0.5\,H_0$, the $\Psi$ field evolves dramatically
($\Delta\Psi/\Psi_0>20\%$--$3700\%$) and violates Planck.

\paragraph{Structural obstruction.}
For all Planck-consistent $m_0$, $w_0$ remains in $[-1.002,-0.986]$
and $w_a$ in $[-0.114, -0.015]$, far from the DESI target
$(w_0=-0.827, w_a=-0.75)$.  The reason is architectural: in the locked
action, the cosmological constant $\rho_\Lambda$ dominates the dark-energy
budget at all $m_0$ where $\Psi$ remains subdominant.
Consequently $w\approx-1$ is essentially forced, independent of $m_0$.

\paragraph{Conclusion.}
Varying $m_0$ alone cannot resolve the DESI $w_0$--$w_a$ tension within
the locked RIFT action.  Resolution requires either removing the bare
cosmological constant or introducing a new Ψ--DE coupling at the action
level --- both constitute Phase~2 (unlocking the action).
This is a clean falsifiable prediction: if DESI Y5 or Euclid confirm
$w_0\neq-1$ at $>3\sigma$, the locked action is conclusively ruled out
and a specific extension is required.
...
\begin{itemize}
  ...
  \item Dark energy equation of state (Sims~109, 111): CPL fit $w_0=-0.992$, $w_a=-0.082$ ($m_0=0.001\,H_0$); $1.3\sigma$ from Planck, $3.6\sigma$ from DESI Y1.  Full $m_0$ scan (Sim~111, 15 values $0.001$--$10\,H_0$) finds minimum achievable DESI tension $3.44\sigma$ at $m_0=0.31\,H_0$ (Planck-consistent); structural obstruction: $\rho_\Lambda$ dominates DE at all Planck-consistent $m_0$, forcing $w\approx-1$ universally.  Resolving the DESI hint requires unlocking the action.
