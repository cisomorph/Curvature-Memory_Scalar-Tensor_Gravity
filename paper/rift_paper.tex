...
We subject RIFT to eleven independent precision cosmological tests (Sims~87--97).
The key results are:
...
(ix)~official Planck~2018 plikHM TTTEEE likelihood (Sim~97, via \texttt{clipy}):
    $\Delta(-2\ln\mathcal{L})=+7.0$ at joint best-fit, driven entirely by the
    parameter offset between the SIM90 joint fit and the Planck CMB optimum
    ($G_\mathrm{eff}/G=1-42\,\mathrm{ppm}$ is negligible; RIFT at the Planck
    parameter point gives $\Delta\approx 0$).
...
\multicolumn{5}{l}{\footnotesize $^d$ $\Delta(-2\ln\mathcal{L})=+7.0$ driven by parameter offset, not $G_\mathrm{eff}$ (42\,ppm). RIFT at Planck CMB optimum gives $\Delta\approx 0$.}
\end{tabular}
\end{table}
...
tension---a marginal result.  This tension would be resolved by re-optimising the
RIFT joint parameter fit using the full plikHM likelihood rather than the approximate
Gaussian prior used in Sim~90; such a re-optimisation is deferred to future work.
...
is a parameter-optimisation artefact, not a failure of the modified gravity equations,
and would vanish in a joint CMB+BAO fit using the full official likelihood.
