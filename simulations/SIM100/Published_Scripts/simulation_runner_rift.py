"""
SIM100 — RIFT 1+1D Time-Domain Galactic Field Evolution
=========================================================
First time-dependent RIFT simulation at galactic scales.
...
M0      = RIFT['m0_kpc_inv']        # 0.1 kpc^-1
L0      = RIFT['Lambda0']           # 0.003
PSI_C   = RIFT['psi_cosmo']         # 0.003
...
def compute_rotation_curve(r_arr, psi, lambda_gal):
    ...
    # T^Psi_00 in RIFT units — convert to M_sun/kpc^3 via c^2/G
...
def run():
    print("=" * 72)
    print("SIM100 v1 — RIFT 1+1D Time-Domain Galactic Evolution")
    print("=" * 72)
    ...
    if not any_cond and not any_blow:
        verdict_str = (
            ...
            "description of RIFT galactic dynamics."
        )
    elif any_blow:
        verdict_str = (
            "TACHYONIC BLOW-UP DETECTED: "
            f"For lambda < {lam_crit_spat:.0f} (spatial instability threshold), "
            "the RIFT field grows without bound from IC Psi=Psi_cosmo. "
        ...
        if not any_pass:
            verdict_str += (
                ...
                "is the outcome of attractive self-coupling in RIFT. "
    ...
    diag['verdict'] = {
        ...
        'physics_note': (
            ...
            "lambda<0 self-coupling, not by RIFT curvature coupling. "
...
def _make_plots(r_arr, all_results, psi_store, vc_store,
    ...
    fig.suptitle('SIM100 — Phase Diagram: RIFT Galactic Condensation', fontsize=12)
    fig.tight_layout()
    ...
    for ic, (lam, delta) in enumerate(cases_d0):
        ...
        if vc is not None:
            ax = axes_v[ic]
            ax.plot(r_arr, vc,    'b-',  lw=2, label='RIFT total')
