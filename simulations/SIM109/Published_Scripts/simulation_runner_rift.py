#!/usr/bin/env python3
"""
SIM109: Dark Energy Equation of State w(z) in RIFT
RIFT: Recursive Intelligence-Field Theory

Solves the full RIFT background equations numerically (Friedmann + modified
...
RIFT background equations (units 8piG=1, H0=1):
...
print(f"H0_RIFT = {np.sqrt(H2_today):.6f}  (should be ~1; LCDM = 1.000000)")
...
# Effective DE = total energy - matter - radiation (treating RIFT as modified H(z))
...
print()
print("="*70)
print("SIM109: Dark Energy Equation of State w(z) in RIFT")
print("="*70)
print(f"Lambda0={Lambda0}, m0={m0} H0, Psi0(z=0)={Psi0_ic}")
print()

print("─── Background Evolution ───")
print(f"{'z':>6} {'Psi(z)':>10} {'dPsi/dx':>10} {'H_RIFT/H0':>12} {'H_LCDM/H0':>12} {'ΔH/H [%]':>10}")
print("-"*65)
...
print(f"  RIFT:         w0 = {w0_fit:+.4f},  wa = {wa_fit:+.4f}")
...
print(f"  RIFT predicts w ≈ -1 (Planck-like), NOT the DESI w0wa hint.")
...
print("SUMMARY")
print("="*70)
print(f"  w0 (RIFT) = {w0_fit:+.6f}")
print(f"  wa (RIFT) = {wa_fit:+.6f}")
print(f"  vs Planck (w0=-1): {'PASS' if planck_pass else 'FAIL'}  ({abs(pull_planck):.2f}σ)")
print(f"  vs DESI Y1:  {desi_tension:.1f}σ tension — RIFT predicts cosmological-constant-like DE")
