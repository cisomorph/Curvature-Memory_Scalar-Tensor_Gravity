#!/usr/bin/env python3
"""
SIM119: RIFT-seeded Fuzzy DM — SPARC sample-wide fit
RIFT: Recursive Intelligence-Field Theory — Phase 2 DM

Extends SIM118 (NGC 2403 single-galaxy fit) to the full SPARC rotation curve
sample (~175 galaxies). Tests the core RIFT-χ prediction:
...
this is a strong non-trivial prediction of the RIFT framework.
...
Data path: ../../../simulation_18_rift_graviton_emit/figures/rift_p7_min_impl (2)/external/sparc_raw/
...
SPARC_DIR = os.path.join(
    ...
    '..', '..', '..',
    'Ordered_Simulations',
    'simulation_18_rift_graviton_emit',
    'figures', 'rift_p7_min_impl (2)', 'external', 'sparc_raw'
)

print("=" * 72)
print("SIM119: RIFT-seeded Fuzzy DM — SPARC sample-wide fit")
print("=" * 72)
...
# ═══════════════════════════════════════════════════════════════════════════════
# PART D: RIFT CONSISTENCY TEST
# ═══════════════════════════════════════════════════════════════════════════════
print("─" * 72)
print("PART D: RIFT consistency — does m₂₂ cluster?")
print("─" * 72)

print(f"""
  RIFT prediction: m_χ = √(2κ) × Ψ̄ = universal constant.
...
if len(log_m22) > 0:
    ...
    print(f"  RIFT consistency criteria:")
...
print(f"""
  ...
  RIFT prediction (m_χ universal): {'CONSISTENT' if all_pass else 'NOT FULLY CONSISTENT'}
...
if verdict == 'PASS':
    note = ("RIFT-seeded fuzzy DM consistent across SPARC sample. "
            "m₂₂ clusters around a single value within 1 dex, supporting "
            "the universal RIFT prediction m_χ = √(2κ)Ψ̄.")
elif verdict == 'PARTIAL':
    note = ("Substantial fraction of SPARC galaxies fit well. "
            "m₂₂ distribution peaked but scatter significant. "
            "RIFT-χ framework viable but m₂₂ universality needs tighter test.")
else:
    note = ("SPARC-wide fit fails. m₂₂ does not cluster; "
            "RIFT-χ framework not consistent with full sample.")
