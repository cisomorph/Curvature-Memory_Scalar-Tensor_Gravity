"""
SIM120 — RIFT Phase 2: Joint DE+DM Background Consistency Test
================================================================
...
Physics:
  ...
  - The RIFT Ψ field sees ρ_m = ρ_baryon + ρ_χ (dust total) regardless
...
# RIFT parameters (locked / SIM113 best-fit)
Lambda0 = 0.003      # Λ₀
...
# F(Ψ) in RIFT: F = ½ + Λ₀Ψ² (M_Pl=1 normalisation)
def F(Psi):
...
# In RIFT Friedmann: 3F(Ψ)H² = ρ_tot
...
#
# RIFT Friedmann (E-frame, Ψ normalised):
...
# ... actually the full RIFT Friedmann with F(Ψ):
# 3F(u)·E²(N) = Ω_m·e^{-3N} + Ω_r·e^{-4N} + ρ_Ψ/ρ_crit,0
# ρ_Ψ/ρ_crit,0 = ½(du/dN)²·E²/? ... need to be careful.
#
# In RIFT: 3F·H² = ρ_m + ρ_r + ρ_Ψ   where ρ_Ψ = ½Ψ̇² + V_J(Ψ)
...
#
# KG equation (from varying action wrt Ψ in RIFT):
...
def E_sq(N, u, up, Omega_m_arr, Omega_r_arr, include_chi=False):
    """
    E²(N) = H²/H₀² from RIFT Friedmann equation.
...
def ode_background(N, y, Omega_m_use):
    ...
    # KG equation for Ψ (in RIFT with F-coupling):
...
if __name__ == '__main__':
    print("=" * 70)
    print("SIM120 — RIFT Phase 2: Joint DE+DM Background Consistency Test")
    print("=" * 70)
    ...
    print(f"  In RIFT, Ψ-EOM: Ψ̈ + 3HΨ̇ + V'_J + 2Λ₀Ψ·R = 0")
    ...
    # ─── κ DERIVATION ────────────────────────────────────────────────────────
    print(f"\n── RIFT DE-DM Link: κ from m_χ = √(2κ)Ψ̄ ──")
    ...
    # ─── DESI H(z) COMPARISON (analytical Flat ΛCDM-like using RIFT E(z)) ──
    ...
    print(f"\n  {'z':>5}  {'H_obs':>8}  {'H_RIFT(w=-1)':>14}  {'H_RIFT(SIM113)':>16}  {'pull':>8}")
    chi2_tot = 0.0
    for z, H_obs, sig in zip(DESI_z_arr, DESI_H_arr, DESI_s_arr):
        a = 1.0/(1.0+z)
        # RIFT E² (frozen Ψ limit): use Ω_m, Ω_r, Ω_DE directly
    ...
    print("  → replacing CDM with χ leaves RIFT Friedmann equation identical")
    print("  → w₀, wₐ shift = 0 at background level (exact, not approximate)")
    print()
    print("  RIFT DE-DM link confirmed:")
