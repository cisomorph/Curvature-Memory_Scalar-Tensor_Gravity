...
Usage
-----
    python simulation_runner_rift.py
...
def G_resum(p):
    # RIFT (resummed) damping: for p >> Λ_mem, G ~ p^{-2(1+γ)}
...
plt.loglog(p, Gr_abs, label=fr"RIFT $G(p)$, $\gamma={gamma}$")
...
def bubble_I(Lc, use_rift: bool):
    ...
    Gq = G_resum(q) if use_rift else G0(q)
    q_col = q[:, None]
    k = np.sqrt(q_col**2 + p_ext**2 + 2.0*q_col*p_ext*cos_th[None, :])
    Gk = G_resum(k) if use_rift else G0(k)
...
plt.plot(L, Ir, label=rf"RIFT $I_{{\rm RIFT}}(\Lambda)$, $\gamma={gamma}$")
plt.xscale("log")
...
# ---------- 3) Console summary ----------
print(f"Tail slopes: baseline ≈ {s0:.3f}, RIFT ≈ {sr:.3f}")
...
# UV acceptance: RIFT slope must be < -2 by a margin
uv_margin = 0.05
if not (sr < -2.0 - uv_margin):
    raise RuntimeError(
        f"UV acceptance failed: RIFT slope {sr:.3f} is not steeper than -2 by > {uv_margin}"
    )

# Cutoff acceptance: RIFT bubble growth must be flatter than baseline
s_base = slope_vs_logL(L, I0)
s_rift = slope_vs_logL(L, Ir)
if not (abs(s_rift) < 0.5 * abs(s_base)):
    raise RuntimeError(
        f"Cutoff acceptance failed: RIFT bubble slope {s_rift:.4g} not sufficiently flatter than baseline {s_base:.4g}"
    )

diag = {
    "uv_tail_slopes": {"baseline": float(s0), "rift": float(sr)},
    "bubble_slope_logLambda": {"baseline": float(s_base), "rift": float(s_rift)},
    "params": {
