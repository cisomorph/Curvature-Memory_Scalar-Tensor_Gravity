#!/usr/bin/env python3
# Sim 82 — RIFT Retarded Kernel
"""
Numerically compute the retarded Green's function G_R(t, r) for the
RIFT scalar field equation in 1+1D spherically-symmetric flat spacetime:
...
ax.semilogy(r, np.abs(G_R_late), label=f"RIFT |G_R| at t={stored_times[min(late_idx,len(stored_times)-1)]:.1f}")
if fit_ok:
    A = np.exp(coeffs[1])
