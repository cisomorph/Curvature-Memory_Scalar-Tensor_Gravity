#!/usr/bin/env python3
# Sim 89 — RIFT Planck TT Likelihood Comparison (Phase 5b)
"""
Closes the final CMB referee gap.

SIM88 compared RIFT vs LCDM C_ell with no noise model (full-sky
noise-free Gaussian chi2). This simulation adds the proper comparison:
RIFT and LCDM CLASS outputs are compared against the Planck 2018
...
Inputs:
  - SIM88 CLASS RIFT C_ell (lensed TT, converted to μK^2)
...
Outputs:
  ...
  Outputs/sim89_cl_vs_planck.png  -- D_l RIFT + LCDM vs Planck theory
...
print("  Loading SIM88 CLASS RIFT C_ell ...", flush=True)
ell_rift, Dl_rift = load_class_cl(sim88["rift_cl"])
print("  Loading SIM88 CLASS LCDM C_ell ...", flush=True)
ell_lcdm, Dl_lcdm = load_class_cl(sim88["lcdm_cl"])

print(f"  RIFT C_ell range: l={ell_rift[0]}..{ell_rift[-1]}, "
      f"D_l at l=2: {Dl_rift[0]:.1f} μK^2")
...
l_max_common = min(ell_rift[-1], ell_lcdm[-1], ell_planck[-1], l_max)
ell_common   = np.arange(l_min, l_max_common + 1)

# Interpolate all onto common grid
Dl_rift_c   = np.interp(ell_common, ell_rift,   Dl_rift)
...
chi2_rift,  chi2_rift_low,  chi2_rift_high,  n_modes_rift  = planck_chi2(Dl_rift_c,  Dl_ref, Nl_c, ell_common, f_sky)
...
n_eff_modes   = n_modes_rift   # approximately same
...
print(f"  RIFT:  chi2={chi2_rift:.2f}  (low-l: {chi2_rift_low:.2f}, high-l: {chi2_rift_high:.2f})")
print(f"  Δchi2 (RIFT−LCDM) = {delta_chi2:+.2f}")
print(f"  f_sky-weighted modes: {n_eff_modes}")
print(f"  chi2_RIFT / n_modes = {chi2_rift/n_eff_modes:.5f}  (<<1 expected for good fit)")

# Fractional residuals
frac_rift = (Dl_rift_c - Dl_planck_c) / np.maximum(Dl_planck_c, 1e-40)
frac_lcdm = (Dl_lcdm_c - Dl_planck_c) / np.maximum(Dl_planck_c, 1e-40)

rms_rift = float(np.sqrt(np.mean(frac_rift**2)))
rms_lcdm = float(np.sqrt(np.mean(frac_lcdm**2)))
print(f"  RMS fractional residual: RIFT={rms_rift*100:.2f}%, LCDM={rms_lcdm*100:.2f}%")

# ── Acceptance ────────────────────────────────────────────────────────────────
delta_max = float(P["acceptance"]["Delta_chi2_RIFT_minus_LCDM_max"])
...
if not passed:
    print(f"  WARNING: RIFT CMB tension Δchi2={delta_chi2:.2f} > {delta_max:.0f} "
...
ax_top.plot(ell_rift,   Dl_rift,   "r-",  lw=1.2, alpha=0.8, label=f"CLASS RIFT (SIM88)  χ²={chi2_rift:.0f}")
...
ax_top.set_title(f"Sim 89 — RIFT vs Planck 2018 TT  |  Δχ²(RIFT−LCDM)={delta_chi2:+.1f}  ({tension_sigma:.1f}σ)",
                 fontsize=10)
ax_top.set_xticklabels([])

ax_bot.semilogx(ell_common, frac_lcdm * 100, "b--", lw=0.8, alpha=0.7, label="LCDM")
ax_bot.semilogx(ell_common, frac_rift  * 100, "r-",  lw=0.8, alpha=0.8, label="RIFT")
...
chi2_per_l_rift = f_sky * (2*ell_common + 1) * np.maximum(
    (Dl_rift_c/np.maximum(Dl_ref, 1e-40)) - 1.0 -
    np.log(np.maximum(Dl_rift_c/np.maximum(Dl_ref, 1e-40), 1e-10)), 0)
...
ax.semilogx(ell_common, chi2_per_l_rift,  "r-",  lw=0.7, alpha=0.8, label="RIFT")
ax.axhline(0, color="k", lw=0.4)
ax.set_xlabel(r"$\ell$"); ax.set_ylabel(r"$\chi^2$ contribution per $\ell$")
ax.set_title(f"Sim 89 — Planck TT χ² per multipole  (total: RIFT={chi2_rift:.0f}, LCDM={chi2_lcdm:.0f})")
...
for ax, (lmin, lmax, title) in zip(axes, bands):
    ...
    ax.plot(ell_common[mask], frac_rift[mask]*100,  "r-", lw=0.8, alpha=0.8, label="RIFT")
    ax.axhline(0, color="k", lw=0.4)
...
diag = {
    "description": (
        "RIFT Planck TT likelihood comparison (Phase 5b). "
    ...
    "RIFT": {
        "chi2_total":  float(chi2_rift),
        "chi2_lowl":   float(chi2_rift_low),
        "chi2_highl":  float(chi2_rift_high),
        "chi2_per_mode": float(chi2_rift / n_eff_modes),
        "rms_fractional_residual": float(rms_rift),
    },
    "comparison": {
        ...
        "interpretation": (
            ...
            "this estimate captures the first-order CMB tension from the RIFT background."
        )
    },
    "artifacts": {
...
print(f"  Δchi2(RIFT−LCDM) = {delta_chi2:+.2f}  ({tension_sigma:.1f}σ)  "
      f"{'PASS' if passed else 'FAIL'}")
