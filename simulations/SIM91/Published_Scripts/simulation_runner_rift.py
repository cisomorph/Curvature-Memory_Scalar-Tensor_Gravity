#!/usr/bin/env python3
# Sim 91 — RIFT Lambda0 Sensitivity Scan
"""
...
Observables computed at each Lambda0:
  ...
  2. D_RIFT / D_LCDM     -- linear growth factor ratio at z=0 (structure growth)
  ...
  6. CMB chi2 (approx)   -- Gaussian parameter chi2 at RIFT background H0/Omm

The simulation answers the key paper question:
  "At what Lambda0 does RIFT become observationally distinguishable from LCDM?"
...
def integrate_growth(Lambda0, N=2000):
    ...
    sol_rift = solve_ivp(rhs, [lna_ini, lna_fin], y0, method="RK45",
                         dense_output=True, rtol=1e-9, atol=1e-12)
    D_rift = float(sol_rift.sol(0.0)[0])
    ...
    return D_rift / D_lcdm if D_lcdm > 0 else 1.0

def sigma8_ratio(D_ratio):
    """sigma_8(RIFT) / sigma_8(LCDM) = D_ratio (linear scaling)."""
    return D_ratio

def H_deviation_rms(Lambda0, z_arr=None):
    """RMS percentage deviation of H_RIFT(z) from H_LCDM(z) over z=0-3."""
    if z_arr is None:
        z_arr = np.linspace(0, 3, 60)
    H_rift = np.array([H_flat(H0, Omm, z, Lambda0) for z in z_arr])
    H_lcdm = np.array([H_flat(H0, Omm, z, 0.0)     for z in z_arr])
    return float(np.sqrt(np.mean(((H_rift - H_lcdm)/H_lcdm)**2))) * 100.0
...
print(f"    D_RIFT/D_LCDM > {thresh['D_ratio_detectable']*100:.1f}%: Lambda0 >= {L0_D}")
...
ax.set_xlabel(r"$\Lambda_0$"); ax.set_ylabel(r"$|1 - D_{\rm RIFT}/D_{\Lambda\rm CDM}|$ [%]")
...
plt.suptitle(f"Sim 91 — RIFT $\\Lambda_0$ Sensitivity Scan\n"
...
ax.semilogy(L0_arr, np.maximum(D_arr*1e4, 1e-3),"b-",  lw=2, label=r"$|1-D_{\rm RIFT}/D_{\Lambda\rm CDM}|$ [×$10^{-4}$%]")
...
ax.set_title("Sim 91 — RIFT observable deviations vs coupling strength", fontsize=11)
...
diag = {
    "description": (
        "RIFT Lambda0 sensitivity scan. Maps G_eff/G, growth factor ratio, "
    ...
    "paper_statement": (
        ...
        "RIFT deviates from LCDM at the ppm level in all observables probed by current "
        "surveys (CMB, BAO, WL). RIFT becomes distinguishable when Lambda0 exceeds "
