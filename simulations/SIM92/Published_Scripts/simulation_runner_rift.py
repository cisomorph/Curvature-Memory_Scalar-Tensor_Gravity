"""
SIM92 — RIFT Non-Linear Structure Growth
=========================================
Computes non-linear P(k) and sigma_8 via HALOFIT (Takahashi+2012) for a
Lambda0 scan, and tests whether RIFT at moderate Lambda0 alleviates the
...
best-fit cosmology (H0=67.59, Omega_m=0.3118). RIFT modifies P(k) through
...
CLASS_EXEC = "/home/aion/Desktop/RIFT_GitHub/CLASS/class"
...
# RIFT field
Psi0     = params["rift_field"]["Psi_ini"]
m0       = params["rift_field"]["m0"]
alpha    = params["rift_field"]["alpha"]
beta     = params["rift_field"]["beta"]
...
def integrate_growth(Lambda0, N=3000):
    ...
    sol_rift = solve_ivp(growth_rhs, [lna_ini, lna_fin], y0,
    ...
    D_rift = sol_rift.y[0, -1]
    D_lcdm = sol_lcdm.y[0, -1]
    return D_rift / D_lcdm, sol_rift, sol_lcdm
...
def linear_pk(k_hMpc, D_ratio=1.0):
    """
    RIFT linear P(k) [(Mpc/h)^3] at z=0.
    = CLASS LCDM P(k) * D_ratio^2  (RIFT growth modification).
    """
...
if kids_window:
    ...
    print(f"  S8 NEVER consistent with KiDS — RIFT cannot alleviate sigma_8 tension")
if nl_amplification:
...
ax1.plot(L0_arr, s8_lin_arr, "o--", color=BLUE,  ms=4, lw=1.2, label=r"RIFT $\sigma_8$ (linear)")
ax1.plot(L0_arr, s8_nl_arr,  "s-",  color=GREEN, ms=4, lw=1.5, label=r"RIFT $\sigma_8$ (HALOFIT)")
...
ax2.plot(L0_arr, S8_nl_arr, "s-", color=GREEN, ms=4, lw=1.5, label=r"RIFT $S_8$ (HALOFIT)")
...
ax2.set_title(r"$S_8$ Tension: RIFT vs Observations", fontsize=11)
ax2.legend(framealpha=0.85)

fig.suptitle(r"RIFT Non-Linear Structure Growth (Sim 92)", fontsize=12, y=1.01)
...
fig.suptitle(r"RIFT Power Spectrum: CLASS linear vs HALOFIT (Sim 92)", fontsize=12, y=1.01)
...
diagnostics = {
    "description": "SIM92 — RIFT non-linear structure growth (CLASS linear P(k) + HALOFIT Takahashi+2012)",
    "linear_pk_method": "CLASS Boltzmann code (SIM90 joint best-fit cosmology), RIFT: P(k)*D_ratio^2",
    "lcdm_reference": {
    ...
    "verdict": {
        ...
        "sigma8_tension":     (
            "RIFT modifies sigma_8 only through the growth factor D_ratio. "
            "At physical Lambda0 (<=0.05), D_ratio deviates from 1 by <0.12%, "
            "producing delta_sigma_8 < 0.001 — far too small to alleviate the "
            "Planck/KiDS sigma_8 tension (~5-6%). RIFT cannot resolve this tension."
        ),
        "paper_statement": (
            "RIFT predicts a sigma_8 suppression relative to LCDM proportional to "
            ...
            "cannot be alleviated by RIFT at physically motivated coupling strengths. "
...
print(f"  RIFT  sigma_8 @ L0=0.003 = {r003['sigma8_nonlin']:.4f}  "
      f"(delta={r003['dsigma8_nl_pct']:+.4f}%)")
print(f"  RIFT  sigma_8 @ L0=0.050 = {r05['sigma8_nonlin']:.4f}  "
      f"(delta={r05['dsigma8_nl_pct']:+.4f}%)")
print(f"  RIFT  S8      @ L0=0.050 = {r05['S8_nonlin']:.4f}")
