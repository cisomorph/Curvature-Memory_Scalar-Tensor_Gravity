"""
SIM93 — RIFT Bayesian Model Comparison
=======================================
Computes the log-Bayes factor ln B(RIFT/LCDM) via the Savage-Dickey density
ratio. LCDM is RIFT at Lambda0=0 (nested model), so:

    B(RIFT/LCDM) = p(Lambda0=0 | data) / p(Lambda0=0 | prior)
...
def H_over_H0(z, Omega_m):
    """Flat LCDM H(z)/H0 (RIFT G_eff correction is ~16 ppm — negligible)."""
    a = 1.0 / (1.0 + z)
...
def savage_dickey(chain_L0, prior_L0_fn, prior_name):
    """
    ln B(RIFT/LCDM) = ln p(L0=0|data) - ln p(L0=0|prior)
    ...
    return {
        ...
        "ln_B_RIFT_LCDM":        ln_B,
        "B_RIFT_LCDM":           math.exp(ln_B) if ln_B is not None else None,
...
def jeffreys_scale(ln_B):
    if ln_B is None:
        return "undefined"
    B = math.exp(ln_B)
    if   B > 150:   return "Decisive evidence for RIFT"
    elif B > 20:    return "Strong evidence for RIFT"
    elif B > 3:     return "Substantial evidence for RIFT"
...
print("=" * 60)
print("SIM93 — RIFT Bayesian Model Comparison")
print("=" * 60)
...
for prior_name, prior_fn in PRIORS.items():
    ...
    if ln_B is not None:
        print(f"    ln B(RIFT/LCDM)               : {ln_B:+.3f}")
        print(f"    B(RIFT/LCDM)                  : {math.exp(ln_B):.3f}")
...
fig.suptitle("SIM93 — RIFT Bayesian Model Comparison: $\\Lambda_0$ Marginal Posteriors",
...
for ax, (prior_name, chain) in zip(axes, all_chains.items()):
    ...
    if sd["ln_B_RIFT_LCDM"] is not None:
        ln_B = sd["ln_B_RIFT_LCDM"]
...
for pn, sd in all_results.items():
    if sd["ln_B_RIFT_LCDM"] is not None:
        ...
        ln_B_vals.append(sd["ln_B_RIFT_LCDM"])
...
if ln_B_vals:
    ...
    ax3.set_ylabel(r"$\ln B(\mathrm{RIFT}/\Lambda\mathrm{CDM})$", fontsize=12)
    ax3.set_title("Savage-Dickey Bayes Factor by Prior Choice", fontsize=11)
    ax3.legend(fontsize=9)
    ax3.text(0.98, 0.95, "RIFT favoured →", transform=ax3.transAxes,
...
diag = {
    "description": "SIM93 — RIFT Bayesian Model Comparison via Savage-Dickey density ratio",
    "method": "Savage-Dickey density ratio: ln B = ln p(L0=0|data) - ln p(L0=0|prior). Valid because LCDM is RIFT at Lambda0=0 (nested models).",
    ...
    "physical_interpretation": (
        ...
        "volume does RIFT have that is not excluded by data? A flat likelihood "
        ...
        "on the Jeffreys scale. RIFT is neither preferred nor excluded."
    ),
    "status": "PASS",
}
...
print(f"  Uniform prior    : ln B = {all_results['uniform']['ln_B_RIFT_LCDM']:+.3f}  ({all_results['uniform']['jeffreys_interpretation']})")
print(f"  Half-Gaussian    : ln B = {all_results['half_gaussian']['ln_B_RIFT_LCDM']:+.3f}  ({all_results['half_gaussian']['jeffreys_interpretation']})")
