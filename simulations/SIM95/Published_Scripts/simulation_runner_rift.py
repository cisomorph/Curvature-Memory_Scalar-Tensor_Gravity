"""
SIM95 — RIFT CMB Polarization EE/TE (CLASS full Boltzmann)
===========================================================
Extends SIM88 (TT) to E-mode polarization. Tests RIFT against the full
CMB polarization signal (EE and TE) using the CLASS Boltzmann code.

Three cosmologies compared:
  A. RIFT BAO-only best-fit (SIM87): H0=68.14, Omega_m=0.294, Lambda0=0.003
  B. RIFT joint CMB+BAO best-fit (SIM90): H0=67.59, Omega_m=0.312, Lambda0=0.008
...
Expected result: RIFT at joint best-fit (B) ≈ LCDM Planck (C) in EE/TE,
...
# ═══════════════════════════════════════════════════════════════════════════════
# 1. RIFT BACKGROUND + GROWTH FACTOR  (identical to SIM88)
# ═══════════════════════════════════════════════════════════════════════════════

def integrate_rift_background(H0, Omega_m, Lambda0, Omega_b,
...
print("=" * 65)
print("SIM95 — RIFT CMB Polarization EE/TE")
print("=" * 65)
...
# ── A: RIFT BAO-only best-fit ─────────────────────────────────────────────────
rp_bao = cosmo['rift_bao_only']
print(f"\n[A] RIFT BAO-only (H0={rp_bao['H0']}, Omm={rp_bao['Omega_m']}, Λ0={rp_bao['Lambda0']})")
a_rift_b, H_rift_b, Psi_rift_b, Geff_rift_b = integrate_rift_background(
    H0=rp_bao['H0'], Omega_m=rp_bao['Omega_m'], Lambda0=rp_bao['Lambda0'],
    Omega_b=rp_bao['Omega_b'])
D_rift_b = integrate_growth(a_rift_b, H_rift_b, Geff_rift_b, rp_bao['Omega_m'], rp_bao['H0'])
D_lcdm_b_ref = integrate_growth(a_rift_b,
    np.sqrt((rp_bao['Omega_m']/a_rift_b**3 + 9.2e-5/a_rift_b**4 +
             (1-rp_bao['Omega_m']-9.2e-5))*rp_bao['H0']**2),
    np.ones_like(a_rift_b), rp_bao['Omega_m'], rp_bao['H0'])
R_bao = float(np.interp(1.0, a_rift_b, D_rift_b)) / float(np.interp(1.0, a_rift_b, D_lcdm_b_ref))
Pk_rift_bao = Pk_lcdm * R_bao**2
pk_bao_path = write_pk(Pk_rift_bao, 'rift_bao')
print(f"  D_RIFT/D_LCDM = {R_bao:.6f}")
cls_bao = run_class(rp_bao['H0'], rp_bao['Omega_m'], rp_bao['Omega_b'],
                    tau_reio, pk_bao_path, 'rift_bao', lmax)

# ── B: RIFT joint best-fit ────────────────────────────────────────────────────
rp_jt = cosmo['rift_joint']
print(f"\n[B] RIFT joint (H0={rp_jt['H0']}, Omm={rp_jt['Omega_m']}, Λ0={rp_jt['Lambda0']})")
a_rift_j, H_rift_j, Psi_rift_j, Geff_rift_j = integrate_rift_background(
    H0=rp_jt['H0'], Omega_m=rp_jt['Omega_m'], Lambda0=rp_jt['Lambda0'],
    Omega_b=rp_jt['Omega_b'])
D_rift_j = integrate_growth(a_rift_j, H_rift_j, Geff_rift_j, rp_jt['Omega_m'], rp_jt['H0'])
D_lcdm_j_ref = integrate_growth(a_rift_j,
    np.sqrt((rp_jt['Omega_m']/a_rift_j**3 + 9.2e-5/a_rift_j**4 +
             (1-rp_jt['Omega_m']-9.2e-5)*1.0)*rp_jt['H0']**2),  # flat LCDM at same params
    np.ones_like(a_rift_j), rp_jt['Omega_m'], rp_jt['H0'])
R_jt  = float(np.interp(1.0, a_rift_j, D_rift_j)) / float(np.interp(1.0, a_rift_j, D_lcdm_j_ref))
Pk_rift_jt = Pk_lcdm * R_jt**2
pk_jt_path = write_pk(Pk_rift_jt, 'rift_joint')
print(f"  D_RIFT/D_LCDM = {R_jt:.6f}")
cls_jt = run_class(rp_jt['H0'], rp_jt['Omega_m'], rp_jt['Omega_b'],
                   tau_reio, pk_jt_path, 'rift_joint', lmax)
...
# RIFT joint vs LCDM Planck (primary test — mirrors SIM88 for TT)
...
print(f"  RIFT joint vs LCDM Planck (l=2–1996):")
print(f"    RMS(ΔTT/TT)    = {rms_TT_jt:.3f}%")
print(f"    RMS(ΔEE/EE)    = {rms_EE_jt:.3f}%")
print(f"    RMS(ΔTE/|TE|)  = {rms_TE_jt:.3f}%")

# RIFT BAO-only vs LCDM Planck (tension check, analogous to SIM89)
if cls_bao is not None:
    ...
    print(f"  RIFT BAO-only vs LCDM Planck (l=2–1996):")
...
print(f"    chi2_EE (RIFT joint vs Planck)  = {chi2_EE_jt:.2f}")
print(f"    Δchi2_EE (RIFT-LCDM)            = {chi2_EE_jt - chi2_EE_lc:+.2f}")

print(f"  TE likelihood (l={l_min}–{l_max_TE}, {n_TE} modes):")
print(f"    chi2_TE (LCDM Planck vs Planck) = {chi2_TE_lc:.2f}")
print(f"    chi2_TE (RIFT joint vs Planck)  = {chi2_TE_jt:.2f}")
print(f"    Δchi2_TE (RIFT-LCDM)            = {chi2_TE_jt - chi2_TE_lc:+.2f}")
...
if cls_bao is not None:
    ...
    chi2_TE_bao, _ = chi2_Gaussian_TE(cls_bao, planck_l, planck_TE,
                                        ...
                                        f_sky, l_min, l_max_TE)
    print(f"\n  BAO-only tension:")
    print(f"    chi2_EE (RIFT BAO-only vs Planck) = {chi2_EE_bao:.1f}")
    print(f"    chi2_TE (RIFT BAO-only vs Planck) = {chi2_TE_bao:.1f}")
...
fig.suptitle("SIM95 — RIFT CMB Polarization EE/TE (CLASS full Boltzmann)", fontsize=13, fontweight='bold')
...
ax.plot(cls_jt['ell'], cls_jt['EE'], 'r--', lw=1.5, alpha=0.8,
        label=f'RIFT joint (χ²={chi2_EE_jt:.0f})')
if cls_bao is not None:
    ax.plot(cls_bao['ell'], cls_bao['EE'], 'g:', lw=1.5, alpha=0.7,
            label=f'RIFT BAO-only (χ²={chi2_EE_bao:.0f})')
...
ax2.plot(cls_jt['ell'], cls_jt['TE'], 'r--', lw=1.5, alpha=0.8,
         label=f'RIFT joint (χ²={chi2_TE_jt:.0f})')
if cls_bao is not None:
    ax2.plot(cls_bao['ell'], cls_bao['TE'], 'g:', lw=1.5, alpha=0.7,
             label=f'RIFT BAO-only (χ²={chi2_TE_bao:.0f})')
...
diag = {
    "description": "SIM95 — RIFT CMB Polarization EE/TE (CLASS full Boltzmann)",
    "growth_ratios": {
        "rift_bao_only": float(R_bao) if R_bao else None,
        "rift_joint":    float(R_jt),
    },
    "rms_deviations_joint_vs_lcdm_planck": {
        ...
        "l_range":    "2–1996",
        "note": "RIFT joint best-fit vs LCDM Planck best-fit. Dominant effect is H0/Omm shift."
    },
    "rms_deviations_bao_only_vs_lcdm_planck": {
        "RMS_TT_pct": rms_TT_bao,
        "RMS_EE_pct": rms_EE_bao,
        "RMS_TE_pct": rms_TE_bao,
        "note": "RIFT BAO-only best-fit vs LCDM. Shows BAO-CMB tension in polarization."
    },
    "planck_likelihood": {
        "EE": {
            "chi2_lcdm_planck": float(chi2_EE_lc),
            "chi2_rift_joint":  float(chi2_EE_jt),
            "chi2_rift_bao_only": float(chi2_EE_bao) if chi2_EE_bao else None,
            "delta_chi2_rift_minus_lcdm": float(chi2_EE_jt - chi2_EE_lc),
            "n_modes": int(n_EE),
            "l_range": f"{l_min}–{l_max_EE}",
        },
        "TE": {
            "chi2_lcdm_planck": float(chi2_TE_lc),
            "chi2_rift_joint":  float(chi2_TE_jt),
            "chi2_rift_bao_only": float(chi2_TE_bao) if chi2_TE_bao else None,
            "delta_chi2_rift_minus_lcdm": float(chi2_TE_jt - chi2_TE_lc),
            "n_modes": int(n_TE),
    ...
    "physical_interpretation": (
        "RIFT at the joint CMB+BAO best-fit reproduces Planck EE and TE within "
        ...
        "equally. The joint-fit RIFT (SIM90) removes this tension for all "
...
print(f"  RIFT joint vs LCDM (l=2–1996):")
print(f"    RMS(ΔTT/TT) = {rms_TT_jt:.2f}%   [SIM88: 2.93%]")
print(f"    RMS(ΔEE/EE) = {rms_EE_jt:.2f}%")
print(f"    RMS(ΔTE/TE) = {rms_TE_jt:.2f}%")
print(f"  Planck EE likelihood:  chi2_LCDM={chi2_EE_lc:.1f}, chi2_RIFT={chi2_EE_jt:.1f}, Δ={chi2_EE_jt-chi2_EE_lc:+.1f}")
print(f"  Planck TE likelihood:  chi2_LCDM={chi2_TE_lc:.1f}, chi2_RIFT={chi2_TE_jt:.1f}, Δ={chi2_TE_jt-chi2_TE_lc:+.1f}")
