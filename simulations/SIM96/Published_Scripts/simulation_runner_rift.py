"""
SIM96 — RIFT RSD / f·sigma_8 Growth Rate
==========================================
Tests RIFT against redshift-space distortion (RSD) measurements from
6dFGRS, SDSS MGS, BOSS DR12, VIPERS, and eBOSS DR16.

Physics:
  RIFT modifies the effective gravitational constant via Λ(Ψ) = Λ₀Ψ²:
    G_eff = G / (1 + 16πΛ(Ψ))
...
Three cosmologies:
  A. RIFT joint CMB+BAO best-fit (SIM90): H0=67.59, Ωm=0.312, Λ0=0.008
  B. RIFT BAO-only best-fit (SIM87): H0=68.14, Ωm=0.294, Λ0=0.003
...
Tests:
  1. Chi² fit at RIFT joint best-fit vs LCDM
  2. Λ0 scan: f·σ8 deviation vs detection threshold
  3. Predictions: first σ8 tension context

Expected result:
  RIFT at Λ0=0.008 → G_eff ≈ 1 (sub-ppm correction) → f·σ8 indistinguishable
  from LCDM at joint best-fit parameters. Δchi²(RIFT−ΛCDM) ≈ 0. RIFT passes RSD.
"""
...
def compute_fsigma8(cosmo, z_grid, sigma8_0):
    ...
    a_arr, H_arr, Psi_arr, Geff_arr = integrate_rift_background(
...
print("=" * 70)
print("SIM96 — RIFT RSD / f·sigma_8 Growth Rate")
print("=" * 70)
...
# Delta chi2: RIFT_joint - LCDM
delta_chi2 = results['rift_joint']['chi2'] - results['lcdm_planck']['chi2']
print(f"Δchi²(RIFT_joint − ΛCDM) = {delta_chi2:+.4f}")
print()
...
base_cosmo = dict(cosmo_cfg['rift_joint'])
...
print("-" * 70)
print("Growth rate f(z) at z_data: RIFT_joint vs LCDM")
print(f"  {'Survey':12s} {'z':>6}  {'f_RIFT':>8}  {'f_LCDM':>8}  {'Δf/f [ppm]':>12}")
...
f_rift  = get_growth_rates(cosmo_cfg['rift_joint'], z_data)
f_lcdm  = get_growth_rates(cosmo_cfg['lcdm_planck'], z_data)
for i in range(len(z_data)):
    df = (f_rift[i] - f_lcdm[i]) / max(abs(f_lcdm[i]), 1e-10) * 1e6
    print(f"  {surveys[i]:12s} z={z_data[i]:.3f}  {f_rift[i]:.5f}  {f_lcdm[i]:.5f}  {df:+12.1f}")
print()
...
S8_rift  = SIGMA8_0 * math.sqrt(cosmo_cfg['rift_joint']['Omega_m'] / 0.3)
...
print(f"  S8 (RIFT joint, sigma8_0={SIGMA8_0}): {S8_rift:.4f}")
print(f"  S8 (LCDM Planck,  sigma8_0={SIGMA8_0}): {S8_lcdm:.4f}")
print(f"  KiDS-1000:  {S8_kids} ± {S8_err} (Asgari+2021)")
print(f"  RIFT pull vs KiDS: {(S8_rift - S8_kids)/S8_err:+.2f}σ")
print(f"  LCDM pull vs KiDS: {(S8_lcdm - S8_kids)/S8_err:+.2f}σ")
print(f"  RIFT cannot alleviate S8 tension (Δsigma_8 < 0.007% at Lambda0=0.003, SIM92)")
print()
...
print(f"  RIFT joint chi2/dof = {chi2_dof_rift:.3f}  (threshold < {acc_max}): {'PASS' if pass_chi2 else 'FAIL'}")
print(f"  RIFT LCDM chi2/dof  = {chi2_dof_lcdm:.3f}")
print(f"  Δchi²(RIFT−ΛCDM)    = {delta_chi2:+.4f}: {'PASS' if pass_delta else 'NOTE'}")
...
fig.suptitle(r'SIM96: RIFT $f\sigma_8$ Growth Rate vs RSD Data', fontsize=14)

# ── Panel 1: f·sigma_8(z) for three cosmologies vs data ──
ax = axes[0]
colors = {'rift_joint': '#1f77b4', 'rift_bao_only': '#ff7f0e', 'lcdm_planck': '#2ca02c'}
labels = {'rift_joint': r'RIFT joint (SIM90: $\Lambda_0=0.008$)',
          'rift_bao_only': r'RIFT BAO-only (SIM87: $\Lambda_0=0.003$)',
          'lcdm_planck': r'$\Lambda$CDM Planck 2018'}
for label in ['lcdm_planck', 'rift_bao_only', 'rift_joint']:
    lw = 2 if label == 'rift_joint' else 1.5
...
ax.text(0.03, 0.05,
        fr'$\chi^2/{dof_val}$ = {chi2_dof_rift:.3f} (RIFT joint)' + '\n' +
...
for label in ['lcdm_planck', 'rift_joint']:
    a_arr_p = 1.0 / (1.0 + z_plot)
    c_p = cosmo_cfg[label]
    a_bg, H_bg, Psi_bg, Geff_bg = integrate_rift_background(
    ...
    lw = 2 if 'rift' in label else 1.5
    ls = '-' if 'rift' in label else '--'
...
ax3.set_title(r'RIFT vs $\Lambda$CDM Growth Rate $f(z)$', fontsize=12)
ax3.text(0.5, 0.05,
         r'$G_{\rm eff}$ correction < 1 ppm at $\Lambda_0=0.008$' + '\n' +
         r'RIFT and $\Lambda$CDM growth rates overlap on this scale',
...
diag = {
    "sim": "SIM96",
    "title": "RIFT RSD / f·sigma_8 Growth Rate",
    "date": "2026-04-09",
    ...
    "delta_chi2_rift_joint_minus_lcdm": float(delta_chi2),
    ...
    "S8_rift_joint": float(S8_rift),
    "S8_lcdm_planck": float(S8_lcdm),
    "S8_kids_pull_rift": float((S8_rift - S8_kids) / S8_err),
