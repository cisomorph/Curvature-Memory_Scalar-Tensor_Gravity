"""
SIM97 — RIFT Full Planck 2018 plikHM TTTEEE Likelihood
=======================================================
...
Three cosmologies:
  A. RIFT joint CMB+BAO best-fit (SIM90): H0=67.59, Omega_m=0.312, Lambda0=0.008
  B. RIFT BAO-only best-fit (SIM87):      H0=68.14, Omega_m=0.294, Lambda0=0.003
  C. LCDM Planck 2018 best-fit:           H0=67.36, Omega_m=0.3153

Key test:
  Delta(-2 ln L) = -2 ln L(RIFT_joint) - (-2 ln L(LCDM))
  PASS if |Delta(-2 ln L)| < 5 on full plikHM TTTEEE.

Expected result:
  RIFT at joint best-fit (A) ≈ LCDM (C): same CLASS spectrum → same likelihood.
  RIFT BAO-only (B) shows large tension (large positive Delta(-2lnL)),
...
# ═══════════════════════════════════════════════════════════════════════════════
# 1. RIFT BACKGROUND (identical to SIM88/95)
# ═══════════════════════════════════════════════════════════════════════════════
...
print("=" * 70)
print("SIM97 — RIFT Full Planck 2018 plikHM TTTEEE Likelihood")
print("=" * 70)
...
d_plik_joint  = results['rift_joint']['m2lnL_plikHM']  - results['lcdm_planck']['m2lnL_plikHM']
d_plik_bao    = results['rift_bao_only']['m2lnL_plikHM'] - results['lcdm_planck']['m2lnL_plikHM']
d_total_joint = results['rift_joint']['m2lnL_total']   - results['lcdm_planck']['m2lnL_total']
d_total_bao   = results['rift_bao_only']['m2lnL_total'] - results['lcdm_planck']['m2lnL_total']

print(f"  Δ(-2 ln L) plikHM:  RIFT joint vs ΛCDM = {d_plik_joint:+.4f}")
print(f"  Δ(-2 ln L) plikHM:  RIFT BAO-only vs ΛCDM = {d_plik_bao:+.4f}")
print(f"  Δ(-2 ln L) total:   RIFT joint vs ΛCDM = {d_total_joint:+.4f}")
print(f"  Δ(-2 ln L) total:   RIFT BAO-only vs ΛCDM = {d_total_bao:+.4f}")
print()
...
print(f"  RIFT joint |Δ(-2 ln L)| = {abs(d_plik_joint):.4f}  (threshold < {acc_max}): {'PASS' if pass_joint else 'FAIL'}")
print(f"  RIFT BAO-only Δ(-2 ln L) = {d_plik_bao:+.1f}  (large tension expected: {'YES' if pass_bao_expected else 'NO'})")
print(f"  G_eff(z=0) at joint best-fit: {results['rift_joint']['Geff_z0']:.8f}  (deviation = {(1-results['rift_joint']['Geff_z0'])*1e6:.1f} ppm)")
print("=" * 70)
print()
...
fig.suptitle('SIM97: RIFT vs Planck 2018 plikHM TTTEEE Likelihood', fontsize=13)

# Panel 1: TT power spectra comparison
ax = axes[0]
colors = {'rift_joint': '#1f77b4', 'rift_bao_only': '#ff7f0e', 'lcdm_planck': '#2ca02c'}
labels = {'rift_joint':   r'RIFT joint ($\Lambda_0=0.008$)',
          'rift_bao_only': r'RIFT BAO-only ($\Lambda_0=0.003$)',
...
diag = {
    "sim": "SIM97",
    "title": "RIFT Full Planck 2018 plikHM TTTEEE Likelihood",
    "date": "2026-04-09",
    ...
    "verdict": {
        ...
        "overall": overall,
        "summary": (
            f"RIFT at SIM90 joint best-fit: Δ(-2lnL)(plikHM TTTEEE) = {d_plik_joint:+.4f} "
            f"vs ΛCDM Planck best-fit. G_eff(z=0) = {results['rift_joint']['Geff_z0']:.8f} "
            f"({(1-results['rift_joint']['Geff_z0'])*1e6:.1f} ppm). "
            f"RIFT BAO-only Δ(-2lnL) = {d_plik_bao:+.1f} (consistent with SIM89 result). "
            f"Verdict: {overall}."
        )
    }
}
