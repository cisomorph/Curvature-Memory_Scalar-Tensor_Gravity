"""
SIM99 — RIFT Galaxy Rotation Curves (v3)
=========================================
Tests whether the locked-action RIFT scalar field can replace dark matter
...
Key question:
  ...
  If not: RIFT at Lambda0=0.003 does not replace dark matter. Galactic-scale
...
M0            = RIFT_P['m0_kpc_inv']          # kpc^-1
L0            = RIFT_P['Lambda0_fiducial']     # 0.003
L0_BOUND      = RIFT_P['Lambda0_cosmo_bound']  # 0.095
PSI_BC_SCAN   = RIFT_P['psi_bc_scan']
LAMBDA_SCAN   = RIFT_P['lambda_gal_scan']
...
def compute_rotation_curve(r_arr, psi, dpsi, m0, lambda_gal, Lambda0, gal):
    """Full RIFT circular velocity: baryons + T^Psi field, G_eff-corrected."""
...
def run():
    print("=" * 70)
    print("SIM99 v3 — RIFT Rotation Curves: Shooting + V(Psi)=(lambda/4)Psi^4")
    print("=" * 70)
    ...
    if any_pass:
        ...
        verdict_str = (
            ...
            f"CONCLUSION: The locked RIFT action does not replace dark matter at "
    ...
    for ig, (gal_name, gal) in enumerate(GALAXIES.items()):
        ...
        ax.plot(r_arr, vc,    'b-',  lw=2.0, label=f'RIFT total ($\\Psi_{{bc}}$={psi_bc:.1e}, $\\lambda$={lam})')
